...
def procEnd(db_con, db_ali, in_pg):
    time.sleep(1)
...
def get_goods_price(in_source):
    ...
    goods_price = str(goods_price).replace('<span class="">','').replace('</span>','').replace('"','').replace('\n','').strip()
    return goods_price
...
def repDesc(in_description):
    ...
    desc = desc.replace('refund','').replace('Refund','')
    return desc.strip()

def proc_asin_parse_brower(in_asin_str,db_con, db_ali, db_ali2, db_price, in_pg, in_pgsite):   
    countryKbn = ""
    asin = ""
    cateidx = ""
    gallery_tmp = ""
    ...
    result = ""
    headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/' + str(random.random()) + ' Safari/537.36', 'Referer': 'https://www.etsy.com'}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("Failed to connect : {}".format(response.status_code))
            return "C05"
    ...
    if str(result).find('No customer reviews') > -1:
        ...
        # with open(os.getcwd() + "/log/reviews_view_"+str(asin)+".html","w", encoding="utf8") as f:
    ...
    ########### Ships from ###########
    ships_from = ""
    ships_from = getparse(result,'"product_details"','<div class="other-info">')
...
def newlist(db_con, db_ali, in_pg, in_ip):
...
def getMemo(in_code):
    ...
    if in_code_no == "D01":
        ...
        rtnMemo = str(in_code) + ' : (price) no price'
    elif in_code_no == "C01":
...
# Goods ###################################################################################
def set_multi(in_pg, in_pgFilename, in_pgKbn, in_pgsite, in_ver, db_con, db_ali, db_ali2, db_price, input_tor):
###########################################################################################
    print('>> set_multi ')
    global cnt_title_tran
    cnt_title_tran = 0
    ...
    # category get
    if input_tor != "Y":
        cateidx = newlist(db_con, db_ali, in_pg, mac_addr())
    else:
        cateidx = newlist(db_con, db_ali, in_pg, currIp)
    ...
    if str(currIp).strip() == "222.104.189.18":
        ...
        version_check(db_con, db_ali, in_ver, in_pgFilename, in_pgKbn)
    ...
    for asin_low in get_asin_list:
        ...
        if allCnt == 1 or allCnt == 50:
            if input_tor != "Y":
                procWork(db_con, db_ali, in_pg, mac_addr())
            else:
                procWork(db_con, db_ali, in_pg, currIp)
        if input_tor == "Y":
        ...
        rtnChk = proc_asin_parse_brower(asin_low, db_con, db_ali, db_ali2, db_price, in_pg, in_pgsite)  
        ...
        db_con.execute(sql)
        print(">> c_Errcnt : {} | d_sold_cnt : {}".format(c_Errcnt, d_sold_cnt))
        ...
        if d_sold_cnt > 10:
            if d_sold_cnt > 5:
                procIpChange(2, input_tor)
                time.sleep(2)               
            print('>> ( d_sold_cnt 10 over ) exit - catecode :' + str(cateidx))
            procLogSet(db_con, in_pg, " ( d_sold_cnt 10 over ) exit - catecode: " + str(cateidx))
            procEnd(db_con, db_ali, in_pg)
...
# stock_out ###############################################################################
def set_stock_out(db_con, db_ali, db_price, in_pg, in_pgFilename, in_pgKbn, in_pgsite, in_ver, in_sql1, in_sql2, in_sql3, input_tor):
###########################################################################################
    print('>> set_stock_out ')
    global cnt_title_tran
    cnt_title_tran = 0
    ...
    for asin_low in get_asin_list:
        ...
        try:
            rtnChk = proc_asin_out_brower(asin_low,db_con,db_ali,db_price,in_pg,in_pgsite)
            print('>> [ rtnChk ] : ' + str(rtnChk))
        ...
        if not rs_row:
            ...
            d_naver_in = rs_row[8]
            print(">> {} | IsDisplay : {} | Del_Naver : {} | stock_ck : {} | regdate : {} | UpdateDate : {} | naver_in : {}".format(d_GoodsCode, d_IsDisplay, d_Del_Naver, d_stock_ck, d_regdate, d_UpdateDate, d_naver_in))
            # stock_cnt = 0
            # if d_stock_ck_cnt != '':
            ...
            if rtnChk_no[:1] == "D":  # sold out
                ...
                db_con.execute(sql)
        print(">> Errcnt : {0} | Errcnt (D17) : {1} | Errcnt (D17) : {2} | d_sold_cnt  : {3} ".format(c_Errcnt, d17_Errcnt, d07_Errcnt, d_sold_cnt))
        ...
        if d_sold_cnt > 10:
            if d_sold_cnt > 5:
                procIpChange(2, input_tor)
                time.sleep(2)               
            print('>> ( d_sold_cnt 10 over ) exit - asin_low :' + str(asin_low))
            procLogSet(db_con, in_pg, " ( d_sold_cnt 10 over ) exit - asin_low: " + str(asin_low))
            procEnd(db_con, db_ali, in_pg)
...
# 재고 체크
def proc_asin_out_brower(in_asin_str,db_con, db_ali, db_price, in_pg, in_pgsite):   
...
# Stock ###################################################################################
def set_updatelist(db_FS, db_con, db_ali, db_ali2, db_price, in_pg, in_pgsite, in_ver, input_tor):
###########################################################################################
    ...
    for asin_low in get_asin_list2:
        ...
        if not rs_row:
            ...
            if rtnChk_no[:1] == "D":  # sold out
                ...
                if str(d_stock_ck) != '9' and rtnChk_no != "D22":
                    sql = "update T_goods set UpdateDate = getdate(), stock_ck = '1', stock_ck_date=getdate(), stock_ck_cnt = stock_ck_cnt + 1 where uid='{0}'".format(rtn_uid)
                    print(">> sql : " + str(sql))
...
# Stock ###################################################################################
def set_stock_multi(db_con, db_ali, db_ali2, db_price, in_pg, in_pgFilename, in_pgKbn, in_pgsite, in_ver, in_sql1, in_sql2, in_sql3, input_tor):
###########################################################################################
