...
def newlist(cursor,ip):
    ...
    if not rows:
        page = 1
        sql = "SELECT top 1 a.amz_cateurl,a.catecode,a.cate_code2, a.big, a.middle, a.small, a.little, a.last, a.depth FROM T_CATEGORY a left outer join update_list b on(a.CateCode=b.catecode) where IsHidden='F' and lastcate=1 and b.catecode is null order by up_date asc"
        ...
        sql = "select c.amz_cateurl,u.catecode,u.now_page,u.cate_code2, u.big, u.middle, u.small, u.little, u.last, u.depth from update_list as u inner join t_category as c on u.catecode = c.catecode where c.site_chk isnull and u.proc_ip = '{0}'".format(ip)
        row = cursor.selectone(sql)
        amzurl = row[0]
        cateidx = row[1]
        page = row[2]
...
def newlist2(db_con, in_ip):
    ...
    if not rows:
        page = 1
        sql = "select top 1 catecode from T_getAsin where catecode not in (select catecode from update_list2 ) order by regdate"
        ...
        if row:
            ...
            if page > 2:
                page = 2
            sql = "update update_list2 set now_page = {0} , regdate=getdate() where proc_ip='{1}'".format(page, in_ip)
            db_con.execute(sql)
