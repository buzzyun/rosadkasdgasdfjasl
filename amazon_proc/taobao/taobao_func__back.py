...
def loginProc_new(in_driver, in_login_id, in_password):
    ...
    #     in_driver.find_element(By.CSS_SELECTOR,'#fm-login-id').send_keys(i)
    #     time.sleep(0.25)
    # print('>> ID OK ')
    # in_driver.find_element(By.NAME,'fm-login-password').click()
    # in_driver.find_element(By.NAME,'fm-login-password').send_keys(Keys.CONTROL + "a")
    ...
    #     in_driver.find_element(By.CSS_SELECTOR,'#fm-login-password').send_keys(i)
    #     time.sleep(0.25)
    # print('>> pass OK ')
    ...
    # time.sleep(2)
    # try:
    #     in_driver.find_element(By.CSS_SELECTOR,'#login-form > div.fm-btn > button').click()
    #     print('>> more click OK ')
...
def demo_login_new(browser):
    ...
    if browser.find_element(By.XPATH,'//*[@id="ot_auth_login"]'):
        browser.find_element(By.XPATH,'//*[@id="ot_auth_login"]').send_keys('1a8389aa-f246-4e24-8e87-de6f89806c6e')
        time.sleep(1)
        ...
        if str(browser.current_url).find('?cmd=Login') > -1:
            if browser.find_element(By.XPATH,'//*[@id="ot_auth_login"]'):
                browser.find_element(By.XPATH,'//*[@id="ot_auth_login"]').send_keys('root')
                browser.find_element(By.XPATH,'//*[@id="ot_auth_password"]').send_keys('38w7sg9BSXgv')
                time.sleep(1)
...
def replaceQueryStringTitle(target) :
    result = target.replace("'","").replace("tmall.com","").replace("- ChinaglobalMall","").replace("- CHINAGLOBALMALL","").replace('�','')
    result = result.replace("★","").replace("◆","").replace("/"," | ").replace(","," ").replace("&lt;","<").replace("&gt;",">").replace("&amp;","&").replace("amp;","").replace("&#39;","`").replace("&quot;","").replace("【","(").replace("】",")").replace('"', '').replace("「","(").replace("」",")")
    return str(result).strip()
...
def proc_asin_parse_brower(gDic, db_con, db_ali, browser, in_pg, in_pgsite, db_price):
    ...
    if result_goods == "":
        ...
        if str(guid) == '' or guid is None:
            guid = ''
            sql = "select isnull(stop_update,'0'), isnull(input_shipping_weight,'0'), uid, isnull(Del_Naver,0), goodscode, DE_title, title, isnull(OriginalPrice,0) from t_goods where ali_no = '{0}'".format(asin)
        else:
            sql = "select isnull(stop_update,'0'), isnull(input_shipping_weight,'0'), uid, isnull(Del_Naver,0), goodscode, DE_title, title, isnull(OriginalPrice,0) from t_goods where uid = {0}".format(guid)
        ...
        if soldoutCheck(result_goods, istmall) == True:
            ...
            optTitleStr = getparse(result_goods,'class="tb-skin"','</div>')
            ...
            if result_goods.find('<title>') > -1:
                ...
                title = getparse(result_goods,'data-title="','"')
            ...
            ##### price #####
            price = getparse(result_goods,'name="current_price" value=','>')
            ...
            imgB = ""
            other_img_set = []
            if result_goods.find('auctionImages    :') > -1 :
                img_str = getparse(result_goods,'auctionImages    :',None)
                img_str = getparse(img_str,'[',']')
                other_img_list = img_str.split(',')
                for ea_other_img in other_img_list:
                    #print(">> ea_other_img : {}".format(ea_other_img))
                    if imgB == "" :
                        imgB = getparse(ea_other_img, '"', '"')
                    else:
                        other_img_set.append(getparse(ea_other_img, '"', '"'))
            ...
            ShopId = getparse(result_goods,'shopId           :',',').replace("'","")
            sellerId = getparse(result_goods,'sellerId         :','')
            sellerId = getparse(sellerId,"'","'")
            #print(">> ShopId : {}".format(ShopId))
            #print(">> sellerId : {}".format(sellerId))

            try:
                moveScroll(browser)
            except Exception as e:
                print(">> moveScroll except (skip) ")
                time.sleep(3)
            else:
                time.sleep(3)

            ### feature ###
            feature = getparse(result_goods,'<ul class="attributes-list">','</ul>')
            if str(feature).find('保修期') > -1:
                rep_feature = getparse(feature,'保修期','</li>')
                feature = str(feature).replace(rep_feature,'')
            if str(feature).find('售后服务') > -1:
                rep_feature2 = getparse(feature,'售后服务','</li>')
                feature = str(feature).replace(rep_feature2,'')
            feature = str(feature).replace('保修期限','').replace('保修期','').replace('保修','').replace('保修期','').replace('全国联保','').replace('售后服务','')
            #print(">> feature : {}".format(str(feature)[:20]))

            ### descript ###
            descript = ""
            if str(price).strip() != "":
                descript = getDescript(browser, result_goods, istmall)
            ...
            if result_goods.find('免运费') == -1:
                taobao_shipping_str = getparse(result_goods,'"J_WlServiceListCon"','</div>')
                if taobao_shipping_str.find("¥</span>") > -1:              
                    taobao_shipping_str = getparse(taobao_shipping_str,'¥</span>','')
                if taobao_shipping_str.find("¥") > -1:              
                    taobao_shipping_str = getparse(taobao_shipping_str,'¥','')
                if taobao_shipping_str.find("<s>") > -1:
                    taobao_shipping_str = getparse(taobao_shipping_str,'','<s>')
                if taobao_shipping_str.find("</li>") > -1:
                    taobao_shipping_str = getparse(taobao_shipping_str,'','</li>')
                #print(">> 유료 배송 : {} | {}".format(asin, taobao_shipping_str))
                result_dic['taobao_shipping'] = 'F'
                if taobao_shipping_str.isdigit():
                    taobao_shipping_fee = float(taobao_shipping_str)
                    if taobao_shipping_fee > 80:
                        print(">> shipping price over: {} | {} ".format(asin, taobao_shipping_fee))
                        return "D11" + " ( " + str(taobao_shipping_fee) + " )"  # shipping_price 80 위안 (SKIP)

            ### option ###  
            option_value_tran_arr = []
            option_source = getparse(result_goods,'valItemInfo      :','});')
            option_source = str(option_source)
            #with open(str(asin)+"_option.html","w",encoding="utf8") as f: 
            #    f.write(str(option_source))

            option_kind_tmp = getparse(result_goods,'class="tb-key tb-key-sku"','<dl class="tb-amount tb-clear">')
            sp_option_kind = str(option_kind_tmp).split('class="J_Prop tb-prop tb-clear')
            ...
            if option_source.find('skuMap     :') == -1:
                option_check = "0"
                print('No Option goods : {} '.format(asin))
                result_dic['OptionKind'] = None
                result_dic['many_option'] = '0'
            else:
                option_sku = getparse(option_source,'skuMap     :','}}')
                option_Map = getparse(option_source,'propertyMemoMap','}')
                option_check = "1"
            ...
            if option_check == "1":
                ...
                for option_ea in sp_option_item:
                    ...
                    option_code = getparse(option_ea,'"','":')
                    option_code = option_code[1:-1]
                    ...
                    option_value = ""
                    opt_set_cnt = 0
                    for ea_option_code in option_code_list :
                        if result_goods.find('<li data-value="'+str(ea_option_code)+'"') > -1 :
                            ea_option_code_value_str = getparse(result_goods,'<li data-value="'+str(ea_option_code)+'"','</li>')
                            ea_option_code_value = getparse(ea_option_code_value_str,'<span>','</span>')
                            ea_option_code_value = ea_option_code_value.replace(r'\x26', '&').replace('&#39;', '`').replace('&amp;', '&').replace('&quot;', '').replace("'", "`").replace('"', '')
                            ...
                            if ea_option_code_value_str.find('background:url(') > -1 :
                                ea_option_code_img = getparse(ea_option_code_value_str,'background:url(',')')
                                img_flag_check = ea_option_code_img.rfind(".")
                                img_flag = ea_option_code_img[img_flag_check:]
                                ea_option_code_img = getparse(ea_option_code_img, None, img_flag)
                                ea_option_code_img = ea_option_code_img + img_flag
                                option_img_dic[ea_option_code_value] = ea_option_code_img
                        else:
                    ...
                    if str(option_value).find("预售") > -1:
                        pass
                        #print(">> Option (사전판매/예약) Skip : {} | {} ".format(ea_option_code, option_value))
                    elif str(option_value).find("保修") > -1:
                        pass
                        #print(">> Option (보증) Skip : {} | {} ".format(ea_option_code, option_value))
...
def set_multi(browser, db_con, db_ali, in_pgKbn, in_ver, goods_dic, db_price):
    ...
    for asin_low in get_asin_list:
        ...
        if rtnChk != "0":  
            if rtnChk_no[:1] == "D" or rtnChk_no == "T02" or rtnChk_no == "T01":
                D_naver_in = ""
                D_goodscode = ""
                ...
                if rs:
                    ...
                    if DIsDisplay == 'T':
                        ...
                        if rtnChk_no[:1] == "D" or rtnChk_no == "T02":
                            if str(D_naver_in) == "1":
...
def proc_asin_out_brower(gDic, db_con, db_ali, browser):
    ...
    try:
        result_goods = str(browser.page_source)
        print(">> page_source ")
        # waitlogo = WebDriverWait(browser, 20).until(EC.presence_of_element_located((By.CLASS_NAME, 'tb-header-content')))
        # if waitlogo:
        #     result_goods = str(browser.page_source)
        #     print(">> page_source ")
    except Exception as e:
    ...
    if str(result_goods).find('id="nocaptcha"') > -1:
        print(">> nocaptcha (input key): {}".format(asin))
        input(">> Sile Key : ")
        time.sleep(5)
        result_goods = str(browser.page_source)

    if str(result_goods).find('id="nocaptcha"') > -1:
        print(">> nocaptcha E99 (exit): {}".format(asin))
        return "E99"
    ...
    if result_goods == "":
        print(">> Connect Error (SKIP): {}".format(goods_url))
        return "C01"
    elif str(browser.current_url).find('detail.tmall.com/item.htm') > -1:
        print(">> tmall goods (SKIP): {}".format(asin))
        return "T01"
    else:
        curr_page = ""
