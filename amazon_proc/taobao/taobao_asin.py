...
#glogin_id = "Kimbyeongwan19"
glogin_id = "Kimbyeongwan21"
...
def newlist(cursor,ip):
    ...
    if not rows:
        page = 1
        sql = "SELECT top 1 a.amz_cateurl,a.catecode,a.update_cnt,a.bcate FROM T_CATEGORY a left outer join update_list b on(a.CateCode=b.catecode) where IsHidden='F' and lastcate=1 and b.catecode is null and sale_ck_new = '1' order by up_date asc"
...
def preProc(result,now_url):
    ...
    if confirmHtmlSource(result, 'login') == True:
        ...
        print('>> curr_url : {} '.format(curr_url))
        if str(curr_url).find('login.taobao.com') > -1:
            print('>> login Error (300) : {} '.format(curr_url))
            time.sleep(300)
        ...
        if result.find('"nid":"') == -1:
            ...
            if result.find('"nid":"') > 0 :
                ...
                while low < len(asin_list):
                    ...
                    if max_price == 'T' and float(view_price) > 1000 :
