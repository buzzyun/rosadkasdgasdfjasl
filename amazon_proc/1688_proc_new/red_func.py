...
def replaceQueryStringTitle(target) :
    result = target.replace("'","").replace("- Taobao","").replace("Taobao","").replace("red","").replace("TAOBAO","").replace("tmall.com","").replace("- ChinaglobalMall","").replace("- CHINAGLOBALMALL","").replace('�','')
...
def procTranConect(browser, in_asin, option_max_count):
    result_tran = ""
    try:
        tran_url = 'https://dev.freeship.co.kr/_GoodsUpdate/title_tran_red.asp?asin={0}'.format(in_asin)
...
# (194 aliexpress 체크) 전체 금지단어 체크 "0":정상단어, "1":금지단어
...
def getMemo(in_code):
    ...
    if in_code_no == "D01":
        ...
        rtnMemo = str(in_code) + ' : (Quantity) minOrder Quantity over'
    elif in_code_no == "D71":
        ...
        rtnMemo = str(in_code) + ' : (Connection C11) Url Connect Error'
    elif in_code_no == "C12":
        rtnMemo = str(in_code) + ' : (Connection C12) Url Connect Error'
    elif in_code_no == "C13":
        rtnMemo = str(in_code) + ' : (Connection C13) Url Connect Error'
    elif in_code_no == "C14":
        rtnMemo = str(in_code) + ' : (Connection C14) Url Connect Error'
    elif in_code_no == "C02":
    ...
    if dic['OptionKind'] == '300' or dic['OptionKind'] == 300:
        ...
        for key,values in dic['option_img_dic'].items():
            if str(values) == '<br>' or str(values) == '':
                ...
                rpl_key = replaceQueryStringOption(key)
                option_name = dic['option_value_dic'][key]
                #option_img_set.append('<br><br><img src="{}"><br><br>'.format(values))
                option_img_set.append('<Font color=blue><pre><b>[({0}){1}]</b></pre></FONT><br><img src="{2}"><br><br>'.format(rpl_key,option_name,values))
...
def setDB_proc(in_asin, dic, db_con, in_pg, in_guid, db_price):
    ...
    goodsinfo_dic['shipping_fee'] = getQueryValue(dic['local_shipping_fee'])
    ...
    goodsinfo_dic['min_ea'] = dic['min_ea']
    goodsinfo_dic['asin_url'] = getQueryValue(dic['asin_url'])
    goodsinfo_dic['site_kbn'] = "'red'"
    ...
    if procFlg == "N":
        ...
        if findChinese(goods_title) == True:
            goodsinfo_dic['confirm_goods'] = 5 # 타이틀 번역후 처리 ( 이후 이미지처리 예정 )
            goodsinfo_dic['title_chk'] = "'5'" 
        else:
            goodsinfo_dic['confirm_goods'] = 1 # img_url 이후처리 (이미지 컨버팅 이후 confirm_goods -> null 변경처리)
        ...
        new_goodscode = getGoodsCode(now_guid, 'Z')
...
def proc_asin_parse_brower(browser, gDic, db_con, db_ali, in_pg, in_pgsite, db_price):
    guid = ""
    min_qty_order = gDic['min_qty_order']
    asin = gDic['asin']
    ...
    goods_url = "https://www.dcbuy.co.kr/mall/detail/" +str(asin)+ "?origin="
    ...
    try:
        ...
        print(">> requests.post Exception : {}".format(error))
        return "C13"
    else:
        ...
        if res_post.status_code == 200:
            ...
            print(">> res_post.status_code : {}".format(res_post.status_code))
            return "C01" + " ( " + str(res_post.status_code) + " ) " 

    result_goods = str(res_post.text)
    result_goods = result_goods.replace('\\r\\n',' ').replace('\\','').replace('  ',' ').strip()
    time.sleep(1)
    ...
    #if result_goods == "" or str(asin_url).find(".com/item.htm?id=") == -1:
    if result_goods == "":
        ...
        if str(guid) == '' or guid is None:
            guid = ''
            sql = "select isnull(stop_update,'0'), isnull(input_shipping_weight,'0'), uid, isnull(Del_Naver,0), goodscode, DE_title, title, isnull(OriginalPrice,0), imgB from t_goods where ali_no = '{0}'".format(asin)
        else:
            sql = "select isnull(stop_update,'0'), isnull(input_shipping_weight,'0'), uid, isnull(Del_Naver,0), goodscode, DE_title, title, isnull(OriginalPrice,0), imgB  from t_goods where uid = {0}".format(guid)
        ...
        # 품절 체크
        if soldoutCheck(result_goods, istmall) == True:
            print('>> sold out (품절 체크) : {}'.format(asin))
            return "D01"
        elif str(result_goods).find('<p class="tb-hint">') > -1: # 품절 체크
            soldStr = getparse(result_goods,'<p class="tb-hint">','</p>')
            if str(soldStr).find('此宝贝已下架') > -1: # 품절 체크
                print('>> sold out : 此宝贝已下架 (품절) {}'.format(asin))
                return "D01"
        else: #class="tb-btn-add" // 加入购物车
            #########################################################################
            result_dic = dict()
            result_goods = str(result_goods)

            ##### title #####
            title = getparse(result_goods,'subject":"','"')
            print(">> title : {}".format(title))
            if title.find('보증금') > -1:
                print('>> [{}] title (보증금) 금지 {}'.format(asin, title))
                return "D47"

            ##### price #####
            price = getparse(getparse(result_goods,'priceRangeList":[',']'),'price":"','"')
            if price == "" or price == "null":
                priceTmp = getparse(result_goods,'quoteType"','}')
                price1 = getparse(priceTmp,'consignPrice":',',')
                price2 = getparse(priceTmp,'jxhyPrice":"','"')
                if price1 == '':
                    price = price2
                else:
                    price = price1
            if str(price) == "0" or str(price) == "" or str(price) == "null":
            ...
            cateList = ['1001','1002','1004','3011','3007','3014','3015','3017','3033']
            # 1044 : 쥬얼리 & 안경 & 시계, 1038 : 신발 & 가방 & 액세사리, 1033 : 의류 & 란제리
            if str(d_bcate) in cateList:
            ...
            ## min_qty_order = 2
            ############################
            cateEaList = ['3016','3020','3021','3050','1011','1013']
            minOrderQuantity = getparse(result_goods,'minOrderQuantity":"','"')
            print(">> minOrderQuantity : {}".format(minOrderQuantity))
            if minOrderQuantity == "":
                minOrderQuantity = 1
            else:
                if str(d_bcate) in cateEaList:
                    if int(minOrderQuantity) > 1:
                        print(">> minOrderQuantity {} Over : {}".format(min_qty_order, minOrderQuantity))
                        return "D23" + " ( " + str(minOrderQuantity) + " ) ( cate: " +str(d_bcate)+ " ) " 
                else:
                    if int(minOrderQuantity) > int(min_qty_order):
                        print(">> minOrderQuantity {} Over : {}".format(min_qty_order, minOrderQuantity))
                        return "D23" + " ( " + str(minOrderQuantity) + " ) " 

            min_ea = int(minOrderQuantity)
            print(">> price : {} * {} = {} ".format(price, min_ea, float(price) * min_ea))
            price = float(price) * min_ea  # 최소수량 * 
            ...
            if str(shipping_weight) == "" or str(shipping_weight) == "0.0":
                shipping_weight = 1
            print(">> shipping_weight: {} * {} = {} ".format(shipping_weight, min_ea, float(shipping_weight) * min_ea))
            shipping_weight = float(shipping_weight) * min_ea  # 최소수량 * 

            ### image ###
            imgB = ""
            other_img_set = []
            if result_goods.find('"images":[') > -1 :
                img_str = getparse(result_goods,'"images":[',']}')
                other_img_list = img_str.split(',')
                for ea_other_img in other_img_list:
                    ea_other_img = getparse(ea_other_img, '"', '"')
            ...
            feature = ""
            # if min_ea == 2:
            #     feature = "* 주문 확인사항 : (2개 배송) 수량1개 선택시 상품 2개 배송 "
            ...
            rep_sp_desc = description.split('<a ')
            for desc_a in rep_sp_desc:
                desctmp = getparse(desc_a,'','</a>')
                if desctmp.find('href="') > -1:
                    repDesc = 'href="' + getparse(desctmp,'href="','"') + '"'
                    description = description.replace(repDesc,'')

            # with open(os.getcwd() + "/log/1688_description"+str(asin)+".html","w",encoding="utf8") as f: 
            #     f.write(str(description))

            link_1688 = ""
            link_1688 = getparse(result_goods,'link":"','"')
            print(">> link_1688 : {}".format(link_1688))
            ...
            result_dic['min_ea'] = min_ea
            result_dic['asin_url'] = link_1688
            ...
            if str(productSkuInfos) == "" or result_goods.find('productSkuInfos":[]') > -1:
                option_check = "0"
                ...
                spSku = str(productSkuInfos).split('}]}')
                ...
                for ea_sku in spSku:
                    ...
                    if sku_id != "":
                        ...
                        if price == "":
                            ...
                            sku_price = price
                        if sku_price == "null" or sku_price == "":
                            sku_price = getparse(getparse(result_goods,'priceRangeList":[',']'),'price":"','"')
                        if sku_price == "null" or sku_price == "":
                            print('>> option no price : {}'.format(sku_price))
                        sku_price = sku_price.replace('"','').replace(',','')
                        sku_price = float(sku_price) * min_ea # 최소수량 * 
                        sku_img = getparse(ea_sku,'skuImageUrl":"','"')
                        sttrSku = ea_sku.split('},{')
                        tmp_attrVlue = ""
                        for ea_attr in sttrSku:
                            attrValue = getparse(ea_attr,'value":"','"')
                            attrValue = attrValue.replace(r'\x26', '&').replace('\xa0', ' ').replace('&#39;', '`').replace('&amp;', '&').replace('&quot;', '').replace("'", "`").replace('"', '')
                            attrValue = replaceQueryStringOption(attrValue)
                            if tmp_attrVlue != "":
                                tmp_attrVlue = tmp_attrVlue + " | " + attrValue
                            else:
                                tmp_attrVlue = attrValue
                        if sku_stock == "0":
                            print(">> [{}] SKIP (품절) {} | {} (수량:{}) ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                        elif sku_price == "":
                            print(">> [{}] SKIP (금액없음) {} | {} (수량:{}) ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                        elif tmp_attrVlue == "":
                            print(">> [{}] SKIP (옵션명없음) {} | {} (수량:{}) ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                        elif tmp_attrVlue.find('사전판매') > -1:
                            print(">> [{}] SKIP (옵션 사전판매) {} | {} (수량:{}) ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                        elif tmp_attrVlue.find('보증') > -1:
                            print(">> [{}] SKIP (옵션 보증) {} | {} (수량:{}) ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                        else:
                            print(">> [{}] {} | {} ({}) {}".format(sku_id, tmp_attrVlue, sku_price, min_ea, sku_stock, sku_img))
                            ...
                            option_value_tran_dic['code'] = str(sku_id).strip()
                            option_value_tran_dic['name'] = tmp_attrVlue.replace(" ","&nbsp;")
                            option_value_tran_arr.append(option_value_tran_dic)

                            #옵션 가격 처리
                            if option_min_price == 0 :
                                option_min_price = sku_price
                            else:
                                if option_min_price > sku_price :
                                    option_min_price = sku_price

                            if option_max_price == 0 :
                                option_max_price = sku_price
                            else:
                                if option_max_price < sku_price :
                                    option_max_price = sku_price
            ...
            if option_check == "":
                ...
                if findChinese(title) == True:
                    ...
                    if str(base_price_tmp) == "":
                        ...
                        sql_i = "insert into T_Category_BestAsin_tran (asin, up_date, price, isTmall, title, descript, option_item) values ('{}',getdate(),'{}','{}',{},'{}',{})".format(asin,'','','N'+getQueryValue(replaceQueryStringTitle(title)), descript, option_item_str)
                ...
                # (사이트 DB) title 금지어 체크 ###########
                forbidden_flag_site = checkForbidden_site(tran_title, catecode, db_con)
                ...
                print(">> originalprice : {} ".format(originalprice))
                # 배대지 배송비
                delievey_fee = 6400 # 기본 5900 + 500 (정밀검수)
                delievey_fee = float(getDeliveryFee(gDic, shipping_weight))
                #print(">> delievey_fee : {} ".format(delievey_fee))

                # 1688 유료배송비
                # 총 상품 수량:0.082kg 배송비:0.50kg당 첫 번째 수수료는 2.50위안이고, 1kg이 추가될 때마다 운임은 5.00위안씩 증가
                local_shipping_fee = 0
                if shipping_weight > 0:
                    local_shipping_fee = (shipping_weight * 5) * float(gDic['py_exchange_Rate']) * 2.1
                else:
                    local_shipping_fee = 5 * float(gDic['py_exchange_Rate']) * 2.1
                print(">> local_shipping_fee : {} * {} = {}".format(local_shipping_fee, min_ea, local_shipping_fee * min_ea))
                local_shipping_fee = local_shipping_fee * min_ea

                ## local_shipping_fee = local_shipping_fee * float(gDic['py_exchange_Rate']) * 2.1
                result_dic['local_shipping_fee'] = local_shipping_fee      
                print(">> local_shipping_fee : {} ".format(local_shipping_fee))
                originalprice2 = originalprice + ((delievey_fee + local_shipping_fee)/2)
                ########### goodsmoney ###########
                goodsmoney = 0
                goodsmoney = getWonpirce(gDic, base_price_tmp)
                print(">> goodsmoney (마진플러스): {} + (배대지) {} + (유료배송) {} ".format(goodsmoney, delievey_fee, local_shipping_fee))
                goodsmoney = goodsmoney + delievey_fee + local_shipping_fee
                ...
                marjin = sale_goodsmoney - originalprice2
                print('>> (sale marjin) : {} - {} = {} ( {} %)'.format(sale_goodsmoney, originalprice2, marjin, (marjin/sale_goodsmoney * 100)))    

                if goodsmoney >= 3000000:
                    print('>> goodsmoney Over : '+str(goodsmoney))
                    return "D09 :" + " ( " + str(goodsmoney) + "원)"

                low_price = float(result_dic['price']) * float(gDic['py_exchange_Rate']) + (int(local_shipping_fee) * (100-tmp_coupon) / 100) + (int(delievey_fee) * (100-tmp_coupon) / 100)
                print('>> low_price : {} (환율 {}) '.format(float(result_dic['price']) * float(gDic['py_exchange_Rate']), float(gDic['py_exchange_Rate'])))
                print('>> taobao_shipping_fee : {} | 배대지 : {} '.format((int(local_shipping_fee) * (100-tmp_coupon) / 100),(int(delievey_fee) * (100-tmp_coupon) / 100)))
                ...
                if option_check == "1":
                    ...
                    if option_item.find("/0") == -1:
                        ...
                        return "D07"
                    if option_item.find("화물") > -1 or option_item.find("계약") > -1 or option_item.find("보증") > -1 or option_item.find("예약") > -1 or option_item.find("경매") > -1 or \
                        option_item.find("무료배송") > -1 or option_item.find("무료 배송") > -1 or option_item.find("환불 불가") > -1 or option_item.find("환불불가") > -1 or \
                        option_item.find("반품 불가") > -1 or option_item.find("반품불가") > -1 or option_item.find("선주문") > -1 or option_item.find("도착 예정") > -1 or \
                        option_item.find("위안") > -1 or option_item.find("보증") > -1 or option_item.find("사전 판매") > -1 or option_item.find("인증서") > -1 \
                        or option_item.find("본인 부담") > -1 or option_item.find("배송 불가") > -1:
...
def version_check(browser, db_con, db_ali, in_ver, in_pgFilename, in_pgKbn):
...
def procEnd(browser, db_con, db_ali, in_pg):
    time.sleep(1)
    #print(' time.sleep(1)')
...
def procWork(db_con, db_ali, in_pg, in_ip):
...
def newlist(browser, db_con, db_ali, in_pg, in_ip):
    cateidx = ""
    ...
    if not rows:
        ...
        if row:
            ...
            try:
                ...
                print('>> Error newlist ')
                # proc end
                procEnd(browser, db_con, db_ali, in_pg)
    else:
...
def get_asinset(in_catecode,db_con):
    asinset = []
    sql = "select top 100 asin, a.isTmall, t.Uid from T_Category_BestAsin as a left join ( select uid, cate_idx, ali_no from t_goods where cate_idx = '" +str(in_catecode)+ "' ) as t on t.ali_no = a.asin where a.cate_idx = '" +str(in_catecode)+ "' "        
...
def set_multi(browser, db_con, db_ali, in_pgKbn, in_ver, goods_dic, db_price):

    pgName = goods_dic['py_pgFilename']
    cateidx = newlist(browser, db_con, db_ali, pgName, currIp)
    ...
    if str(currIp).strip() == "222.104.189.18":
        ...
        version_check(browser, db_con, db_ali, in_ver, pgName, in_pgKbn)
    ...
    for asin_low in get_asin_list:
        ...
        rtnChk = proc_asin_parse_brower(browser, goods_dic, db_con, db_ali, pgName, "red", db_price)  
                ...
                if rs:
                    ...
                    if DIsDisplay == 'T':
                        if rtnChk_no == "D03":  # Forbidden 금지어일 경우 판매불가 상품처리                            
                            D03_proc(db_con, Duid)
                            print(">> Forbidden 금지어 판매불가 상품처리: " + str(asin_low))
                        else:
                            soldout_proc(db_con, Duid) 
        ...
        end_check_proc(browser, db_con, db_ali, rtnChk_no, c_Errcnt, asin_low, pgName)
...
def get_update_goods(in_site, db_FS, db_con):
    ...
    sql = sql + " where flg_chk ='0' and sitecate = '" +str(in_site)+"' and isnull(istmall,'') <> 'T' "
...
# Forbidden 금지어일 경우 판매불가 상품처리
def D03_proc(db_con, uid):
    print('>> Forbidden 금지어일 경우 판매불가 상품처리 ')
    sql_u1 = "UPDATE t_goods SET isdisplay='F',IsSoldOut='T', Stock='0', Del_Naver='1', NAVER_stockout='2', stop_update='1', stock_ck = '1', stock_ck_date=getdate() where uid = {0}".format(uid)
    db_con.execute(sql_u1)

    sql_u2 = "update t_goods_sub set IsDelContentFile = 'T' where uid = {0}".format(uid)
    db_con.execute(sql_u2)

# 품절처리 
def soldout_proc(db_con, uid):
    sql = "update T_goods set IsDisplay='F', IsSoldOut='T', Stock='0', stock_ck = '1', stock_ck_date=getdate(), UpdateDate=getdate() where uid='{0}'".format(uid)
    print(">> sql : " + str(sql))
    db_con.execute(sql)

def end_check_proc(browser, db_con, db_ali, rtnChk_no, c_Errcnt, asin_low, pgName):
    if rtnChk_no[:1] == "C":
        if c_Errcnt > 15:
            ...
            procEnd(browser, db_con, db_ali, pgName)

def rtn_proc(db_con, asin_low, rtnChk_no, guid, asin, in_pgKbn):
    ...
    if not rs_row:
        ...
        if rtnChk_no[:1] == "D":  # sold out
            if d_IsDisplay == 'T':
                if rtnChk_no == "D03":  # Forbidden 금지어일 경우 판매불가 상품처리
                    D03_proc(db_con, guid)
                    print(">> Forbidden 금지어 판매불가 상품처리: " + str(asin_low))
                else:
                    print('>> [' + str(asin) + '] setDisplay (품절 처리) :' + str(guid))  
                    soldout_proc(db_con, guid)                            
                    print(">> 품절 처리 OK : " + str(asin_low))
            ...
            if in_pgKbn.find('stock_out') > -1:
                sql = "update T_goods set stock_ck = '2' where uid='{0}'".format(guid)
            else:
            ...
            if in_pgKbn.find('stock_out') > -1:
                sql = "update T_goods set stock_ck = '0' where uid='{0}'".format(guid)
            else:
...
# Stock ###################################################################################
def set_updatelist(browser, db_FS, db_con, db_ali, in_pgKbn, in_ver, goods_dic, db_price, in_Site):
###########################################################################################
    ...
    # asin get
    get_asin_list2 = []
    get_asin_list2 = get_update_goods(in_pgKbn, db_FS, db_con)
    ...
    for asin_low in get_asin_list2:
        ...
        try:
            ...
            print('>> set_updatelist Exception Error : {}'.format(asin_low))
            if rtnChk == "":
                rtnChk = "E01"
        else:
            print('>> -- proc_asin_parse_brower (OK) -- ')
        ...
        elif rtnChk_no[:1] == "C":  # Connection Error
            c_Errcnt = c_Errcnt + 1
        ...
        # 결과 DB update 처리
        rtn_proc(db_con, asin_low, rtnChk_no, guid, asin, in_pgKbn)
        ...
        if rtnChk_no == "P01" or rtnChk_no[:1] == "C" or rtnChk_no[:1] == "Q" or rtnChk_no[:1] == "E":
            sql = "update amazon_goods_update set flg_chk = '0', regdate = regdate + 1 where guid='{0}' and sitecate = '{1}'".format(guid, in_Site)
            print(">> sql : " + str(sql))
            print(">> amazon_goods_update  : " + str(guid))
            db_FS.execute(sql)
        else:
            if rtnChk_no == "T01":
                sql = "update amazon_goods_update set istmall = 'T', memo = 'Tmall goods ' where guid = '{0}' and sitecate = '{1}'".format(guid, in_Site)
                print(">> sql : " + str(sql))
                print(">> amazon_goods_update update : " + str(guid))
                db_FS.execute(sql)
            else:
                sql = "update amazon_goods_update set flg_chk = '1', upddate = getdate() where guid='{0}' and sitecate = '{1}'".format(guid, in_Site)
                print(">> sql : " + str(sql))
                print(">> amazon_goods_update  : " + str(guid))
                db_FS.execute(sql)
...
# Stock ##################################################################################################
def set_stock_multi(browser, db_con, db_ali, in_pgKbn, in_ver, goods_dic, db_price):
##########################################################################################################
...
# stock_out ###############################################################################
def set_stock_out(browser, db_con, db_ali, in_pgKbn, in_ver, goods_dic):
    print('>> set_stock_out ')
    pgName = goods_dic['py_pgFilename']
    if str(currIp).strip() == "222.104.189.18":
        print('>> version_check (Skip) local : ' + str(currIp))
    ...
    for asin_low in get_asin_list:
        ...
        rtnChk = proc_asin_out_brower(goods_dic, db_con, db_ali, browser)
        print('>> [ rtnChk ] : ' + str(rtnChk))
        ...
        # 결과 DB update 처리
        rtn_proc(db_con, asin_low, rtnChk_no, rtn_uid, rtn_asin, in_pgKbn)
        end_check_proc(browser, db_con, db_ali, rtnChk_no, c_Errcnt, asin_low, pgName)

    return "0"

# 재고 체크
def proc_asin_out_brower(gDic, db_con, db_ali, browser):
    ...
    if result_goods == "":
        print(">> Connect Error (SKIP): {}".format(goods_url))
        return "C01"
    elif str(browser.current_url).find('tmall') > -1:
        print(">> tmall goods (SKIP): {}".format(asin))
        return "T01"
    else:
        db_Weight = "0"
        DB_stop_update = "0"
