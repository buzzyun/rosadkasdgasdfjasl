...
if __name__ == '__main__':
    ...
    link_url = "https://www.dcbuy.co.kr/mall/detail/670749547861?origin="

    proc_flg = "0"
    while proc_flg == "0":
        link_url = input(">>url :").strip()
        if link_url == "":
            link_url = input(">>Please Input url :").strip()

        asin = getparse(link_url,'/detail/','')
        if asin.find('?') > -1:
            asin = getparse(asin,'','?')
        # try:
        #     print(">> link_url : {}".format(link_url))
        #     browser.get(link_url)
        # except Exception as e:
        #     print(">> browser.get except ")
        # else:
        #     time.sleep(random.uniform(4,8))
        #     reqult = browser.page_source
        #     print(">> asin : {}".format(asin))

        #     with open(os.getcwd() + "/log/result_ctbuy1688_" +str(asin)+ ".html","w",encoding="utf8") as f: 
        #         f.write(str(reqult))
        # print(">>")
