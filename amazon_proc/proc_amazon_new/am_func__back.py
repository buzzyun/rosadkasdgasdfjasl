...
def soldout_check_2(in_source, inCountry):
    ...
    if str(check).strip() != "":
        ...
        # if inCountry == "DE" and str(rtn_price).find('USD') == -1:
        #     print('>> USD 설정안되었음 . error')
        #     rtn_price = "-1"
        # else:
...
def get_newgoodscode(db_con, now_guid, pgsite):
    ...
    if pgsite == 'mall' or pgsite == 'MALL':
        ...
        new_goodscode = getGoodsCode(now_guid, 'KF', 9)
...
def getDescription(in_soup, inCountryKbn):
    ...
    if str(in_soup).find('<h2>Product Description</h2>') > -1 and str(description_str).find('<h2>Product Description</h2>') == -1:

        description_tmp = getparse(str(in_soup), '<h2>Product Description</h2>', '')
        if str(description_tmp).find('<div id="detailBullets') > -1:
            ...
            description_str = getparse(str(description_tmp), '', '<a id="customerReviews"')
        ...
        description_str = str(description_str).replace('Amazon.co.jp', '')
    ...
    if str(description_str).strip() == "":
        ...
        if str(description_str).find('Customers who bought this item also bought') > -1:
            ...
            print(">> description cut(5-2) ")
        if str(description_str).find('Products related to this item') > -1:
    ...
    if str(description_str).strip() != "":
        description_str = str(description_str).replace('Amazon.co.jp', '').replace('Amazon.com', '')
        description_str = str(description_str).replace('<noscript>', '').replace('</noscript>', '')
...
## parsing_detail #######################################
def parsing_detail(in_soup, in_asin, goods, db_con, db_ali, db_price, in_drive, in_pg, in_countryKbn, in_pgsite, manage_dic):
    global cnt_title_tran
    currSymbol = ""
    stock_flg = "0"
    ...
    try:
        ...
        print('>> exception checkForbidden_new ')
        checkIP()
        time.sleep(10)
        procEnd(db_con, db_ali, in_drive, "")
    ...
    description_str = description_str.replace(in_asin,'').replace('warranty ','**')
    if str(description_str).find("Medical Device Certification Number") > -1:
    ...
    if str(in_soup).find('id="availability"') > -1:
        ...
        if stock_chk.find('Only ') > -1:
            ...
            stock_tmp = '0'
        elif stock_chk.find('In Stock.') > -1:
            stock_tmp = '99999'
    ...
    if option_pos == -1:
        ...
        if flg_opt_chk == "1":
            ...
            # if str(now_source).find('"ajaxUrlParams" :') > -1:
            #     ajaxUrlParams = getparse(str(now_source),'"ajaxUrlParams" :',',')
            #     ajaxUrlParams = getparse(ajaxUrlParams,'"','"')
            #     onurl = 'https://' + str(siteName) + '/gp/twister/dimension?isDimensionSlotsAjax=1&asinList='+str(itemaisnTmp)
            #     onurl = onurl + ajaxUrlParams + "&deviceType=web&showFancyPrice=false&twisterFlavor=twisterPlusDesktopConfigurator"
            # else:
            #     onurl = 'https://' + str(siteName) + '/gp/twister/dimension?isDimensionSlotsAjax=1&asinList='+str(itemaisnTmp)+'&vs=1&productTypeDefinition=DISHWARE_BOWL&productGroupId=home_display_on_website&parentAsin=' +str(parentasin)+ '&isPrime=0&isOneClickEnabled=0&deviceType=web&showFancyPrice=false&twisterFlavor=twisterPlusDesktopConfigurator'
            ...
            #     if str(opt_html_str) == "" or str(opt_html_str).find('captcha') > -1 or str(opt_html_str).find('continue-shopping') > -1:
            #         print(">> continue-shopping or captcha")
            #         flg_opt_chk = "2"
            #     else:
            #         cntj = 0
            #         optFirstChkFlg = "0"
            ...
            #                 itemOrg = getparse(str(opt_html_str), item_optcode,'}')

            #                 if itemOrg.find('"price":"') > -1:
            #                     itemTemp = getparse(str(itemOrg), '"price":"','"}')
            #                     itemTemp = itemTemp.replace("'", "").replace(',', '').replace(currSymbol, '').strip()
            #                 else:
            #                     itemOrg = getparse(str(itemOrg), 'span id=','')
            #                     if itemOrg.find("<\\/span>") > -1:
            #                         itemOrg = getparse(str(itemOrg), '','<\\/span>')
            #                     else:
            #                         itemOrg = getparse(str(itemOrg), '','<\/span>')
            #                     if itemOrg.find('Currently unavailable') > -1:
            #                         print(">> [{}] Currently unavailable ".format(item_optcode))
            #                     itemTemp = getparse(str(itemOrg), 'a-size-mini','')
            #                     itemTemp = getparse(str(itemTemp), '">','')
            #                     if itemTemp.find('from') > -1:
            #                         itemTemp = getparse(str(itemTemp), 'from','')
            #                     itemTemp = itemTemp.replace("'", "").replace(',', '').replace(currSymbol, '').strip()
            #                 if itemTemp == "":
            #                     itemTemp = getparse(str(itemTemp), str(contry_cur),'')
            #                     itemTemp = itemTemp.replace("'", "").replace(',', '').replace(currSymbol, '').strip()

            #                 ea_opt_price = replace_currency(itemTemp)

            #                 if str(ea_opt_price).strip() == "":
            #                     print(">> [{}] ea_opt_price (없음) : {} ".format(item_optcode,ea_opt_price))
            #                     print(">> itemOrg : {} ".format(itemOrg))
            #                 else:
            ...
            #                             if float(ea_opt_price) > 1 and float(ea_opt_price) < float(base_price):
            #                                 base_price = float(ea_opt_price)
            #                         elif in_countryKbn == "US" or in_countryKbn == "DE" or in_countryKbn == "UK":
            #                             if float(ea_opt_price) > 0.1 and float(ea_opt_price) < float(base_price):
            #                                 base_price = float(ea_opt_price)   
            ...
            #             flg_opt_chk = "2"
            #             print('>> Option Price check please (2) ')
            #             path_file = os.getcwd()
            #             with open(path_file + "/result_price_option2.html","w",encoding="utf8") as f: 
            ...
            if flg_opt_chk == "2":
                ...
                # if str(now_source).find('<span class="a-dropdown-container">') > -1:
                #     #print(">> dropdown option ")
                #     dropdown_elem = getparse(str(now_source),'<span class="a-dropdown-container">','</option>')
                #     if str(dropdown_elem).find('Quantity:') == -1:
                #         dropdown_elem = getparse(str(dropdown_elem),'<select name="','')
                #         dropdown_elem = getparse(str(dropdown_elem),'id="','"')
                #         print(">> dropdown_elem : {}".format(dropdown_elem))
                #         if dropdown_elem:
                #             try:
                #                 dropdown_btn = in_drive.find_element(By.CSS_SELECTOR,'#'+str(dropdown_elem))
                #                 if dropdown_btn:
                #                     dropdown_btn.click()
                #                     time.sleep(2)
                #                     dropdown_elem_1 = getparse(str(in_drive.page_source),'<span class="a-dropdown-container">','</select>')
                #                     dropdown_elem_1 = getparse(dropdown_elem_1,'class="dropdownAvailable"','</option>')
                #                     dropdown_elem_1 = getparse(dropdown_elem_1,'id="','"')
                #                     print(">> dropdown_elem_1 : {}".format(dropdown_elem_1))
                #                     if dropdown_elem_1:
                #                         in_drive.find_element(By.CSS_SELECTOR,'#'+str(dropdown_elem_1)).click()
                #                         time.sleep(2)
                #             except Exception as ex:
                #                 print(">> dropdown_btn exception ")

                # if str(now_source).find('<form id="twister"') > -1:
                #     btnopt_elem = getparse(str(now_source),'<form id="twister"','</form>')
                #     sp_btnopt = btnopt_elem.split("<ul class=")
                #     rowbtncnt = 0
                #     for eaopt_btn in sp_btnopt:
                #         if rowbtncnt == 0:
                #             pass
                #         else:
                #             optbtn = getparse(str(eaopt_btn),'<li','</ul>')
                #             optbtn = getparse(str(optbtn),'','class="swatchAvailable"')
                #             optbtnId = getparseR(str(optbtn),'id="','"')
                #             print(">> optbtnId : {}".format(optbtnId))
                #             if optbtnId:
                #                 try:
                #                     in_drive.find_element(By.CSS_SELECTOR,'#'+str(optbtnId)).click()
                #                     time.sleep(0.5)
                #                 except Exception as ex:
                #                     print(">> opt_btn click exception ")
                #         rowbtncnt = rowbtncnt + 1
                ...
                # ajaxUrlTmp = getparse(str(now_source),'"rhfHandlerParams":{','}')
                # currentPageType = getparse(str(ajaxUrlTmp),'currentPageType":"','"')
                # currentSubPageType = getparse(str(ajaxUrlTmp),'currentSubPageType":"','"')
                # excludeAsin = getparse(str(ajaxUrlTmp),'excludeAsin":"','"')
                # dpEnvironment = getparse(str(now_source),'"dpEnvironment" : "','"')
                optSkipCnt = 0
                for item_optcode in subasin_set:
                    priceToSet = ""
                    ...
                    if opt_source_code.status_code == 200:
                        ...
                        # path_file = os.getcwd()
                        # with open(path_file + "/log_opt/result_price_option2_"+str(item_optcode)+".html","w",encoding="utf8") as f: 
                        #     f.write(str(opt_html_str))
                        #     time.sleep(0.5)
                        ...
                        if priceToSet == "" and str(opt_html_str).find('twister-plus-price-data-price') > -1:
                            ...
                            priceToSet = getparse(priceToSet,'value=','').replace('\\', '').replace('"', '').strip()
        ...
        if option_flg == "1" and opt_val_cnt > 1 and opt_price_cnt == 1:
            ...
            if ea_price_last_asin != "":
                ...
                if str(result_2).find('id="productTitle"') == -1:
                    ...
                    ########### image (해당 옵션 이미지 변경) ###########
                    flg_noimg = "0"
                    img_items_str = getparse(str(result_2), "'colorImages':", "</script>")
                    goods['naver_img'] = None

                    if img_items_str.find('"large":"') == -1:
                        print('>> no Img ')
                        return "D19"  # No img     

                    img_items = img_items_str.split('"large":"')
                    imgset = []
                    low = 1
                    while low < len(img_items):
                        ea_img = getparse(img_items[low], None, '"')
                        imgset.append(ea_img)
                        if low == 1:
                            if str(ea_img).find('21EvqGR5jyL.') > -1:
                                print('>> Noimg : ' + str(ea_img))
                                flg_noimg = "1"
                                break
                            if str(ea_img).find('01MKUOLsA5L.') > -1:
                                print('>> Noimg : ' + str(ea_img))
                                flg_noimg = "1"
                                break
                            if str(ea_img).find('21i3Jn-5C5L.') > -1:
                                print('>> Noimg : ' + str(ea_img))
                                flg_noimg = "1"
                                break

                            naver_img = ""
                            if str(img_items_str).find('{"hiRes":"') > -1:
                                naver_img = getparse(str(in_soup), '{"hiRes":"', '"')
                                if str(naver_img) != "":
                                    goods['naver_img'] = naver_img
                                    print('>> naver_img : ' + str(naver_img))

                            if str(naver_img) == "":  # hiRes 이미지가 없을경우 편집
                                if str(ea_img).find('.jpg') > -1 or str(ea_img).find('.JPG') > -1:
                                    ea_img = getparse(ea_img, '', '._') + '.jpg'
                                    print('>> (No naver) ea_img (edit): ' + str(ea_img))

                            goods['mainimage'] = ea_img
                            print('>> mainimage : ' + str(ea_img))

                        # print('>> ' +str(low)+')image :' + str(ea_img))
                        low += 1
...
def replace_currency_opt(in_string):
    ...
    result_str = result_str.replace('Â\xa0', '').replace('Â', '').strip()
...
def proc_asin_parse_brower(in_asin_str, db_con, db_ali, db_price, in_drive, in_pg, in_pgsite, manage_dic):   
    ...
    if str(detail_soup).find('No customer reviews') > -1:
        ...
        rtn_reviews_arr = get_Review_new(str(detail_soup), asin)
    ...
    rtnDetail = ""
    rtnDetail = parsing_detail(str(detail_soup), asin, goods, db_con, db_ali, db_price, in_drive, in_pg, countryKbn, in_pgsite, manage_dic)
    if rtnDetail != "0":
...
# 재고 체크
def proc_asin_out_brower(in_asin_str,db_con, db_ali, db_price, in_drive, in_pg, in_pgsite, manage_dic):   
    countryKbn = getCountryInfo(in_pgsite,"1")
    in_tor = manage_dic['py_tor'] 
    chkCode = ""
    asin = ""
    ...
    print(">> Browser Count : {}".format(len(in_drive.window_handles)))
    main = in_drive.window_handles
    last_tab = in_drive.window_handles[len(main) - 1]
    if str(len(main)) != "1":
        for handle in main:
            if handle != last_tab:
                in_drive.switch_to.window(window_name=last_tab)
                in_drive.close()
            in_drive.switch_to.window(window_name=handle)

        print(">> Browser Close : {}".format(len(in_drive.window_handles)))
        procLogSet(db_con, in_pg, ">> ( OpenBrowser Close proc) : " + str(in_asin_str)) 
...
def version_check(db_con, db_ali, in_drive, in_ver, in_pgFilename, in_pgKbn, in_pgsite):
...
def newlist(db_con, db_ali, in_drive, in_pg, in_ip, in_pgsite):
...
def endProc(db_con, db_ali, in_drive, rtnChk_no, in_pg, in_pgsite, asin_low, c_Errcnt, d17_Errcnt, d07_Errcnt):
    if rtnChk_no[:1] == "C":
        if in_pgsite != "uk" :
            set_new_tor_ip()
            checkCurrIP()
    ...
    if rtnChk_no == "D17":
        ...
        time.sleep(2)
        if d17_Errcnt > 5:
            print('>> ( d17_Errcnt 5 over ) exit :' + str(asin_low))
            procLogSet(db_con, in_pg, " ( d17_Errcnt 6 over ) exit : " + str(asin_low))
            procEnd(db_con, db_ali, in_drive, in_pg)
    ...
    if rtnChk_no == "D07":
        ...
        time.sleep(1)
        if d07_Errcnt > 5:
            print('>> ( D07_Errcnt 5 over ) exit :' + str(asin_low))
            procLogSet(db_con, in_pg, " ( D07_Errcnt 5 over ) exit : " + str(asin_low))
            procEnd(db_con, db_ali, in_drive, in_pg)
...
# Goods ###################################################################################
def set_multi(in_drive, in_ver, db_con, db_ali, db_price, manage_dic):
###########################################################################################
    print('>> set_multi ')
    global cnt_title_tran
    cnt_title_tran = 0
    ...
    if str(currIp).strip() == "222.104.189.18":
        ...
        version_check(db_con, db_ali, in_drive, in_ver, in_pgFilename, in_pgKbn, in_pgsite)

    # category get
    if in_tor == "Y": # 토루 사용
        cateidx = newlist(db_con, db_ali, in_drive, in_pg, ip, in_pgsite)
    elif in_tor == "V": # vpn 사용
        cateidx = newlist(db_con, db_ali, in_drive, in_pg, mac_addr(), in_pgsite)
    else:
        cateidx = newlist(db_con, db_ali, in_drive, in_pg, ip, in_pgsite)
    ...
    for asin_low in get_asin_list:
        ...
        if allCnt % 5 == 0:
            if in_tor == "Y": # 토루 사용
                procWork(db_con, db_ali, in_drive, in_pg, ip)
                set_new_tor_ip()
                checkCurrIP()
                time.sleep(2)
            elif in_tor == "V": # vpn 사용
                procWork(db_con, db_ali, in_drive, in_pg, mac_address)
                time.sleep(4)
            else:
                procWork(db_con, db_ali, in_drive, in_pg, ip)
                set_new_tor_ip()
        ...
        time.sleep(random.uniform(0.5,1))
        rtnChk = proc_asin_parse_brower(asin_low, db_con, db_ali, db_price, in_drive, in_pg, in_pgsite, manage_dic)  
        ...
        endProc(db_con, db_ali, in_drive, rtnChk_no, in_pg, in_pgsite, asin_low, c_Errcnt, d17_Errcnt, d07_Errcnt)

    return "0"


# stock_out ###################################################################################
def set_stock_out(db_con, db_ali, db_price, in_drive, in_ver, manage_dic):
###########################################################################################
    print('>> set_stock_out ')
    global cnt_title_tran
    cnt_title_tran = 0
    ...
    for asin_low in get_asin_list:
        ...
        try:
            rtnChk = proc_asin_out_brower(asin_low,db_con,db_ali,db_price,in_drive,in_pg,in_pgsite, manage_dic)
            print('>> [ rtnChk ] : ' + str(rtnChk))
        except Exception as ex:
...
# Stock ###################################################################################
def set_updatelist(db_FS, db_con, db_ali, db_price, in_drive, in_ver, manage_dic):
###########################################################################################
...
# Stock ###################################################################################
def set_stock_multi(db_con, db_ali, db_price, in_drive, in_ver, manage_dic):
###########################################################################################
...
#### Old Goods #####################################################################################################################
def set_old_multi(db_con, db_ali, db_price, in_drive, in_ver, manage_dic):
####################################################################################################################################
    ...
    for asin_low in get_asin_list:
        ...
        try:
            rtnChk = proc_asin_parse_brower(asin_low,db_con,db_ali,db_price,in_drive,in_pg,in_pgsite)
            print('>> [ rtnChk ] : ' + str(rtnChk))
        except Exception as ex:
            print('>> set_multi Exception Error : ' + str(ex))
            print('>> asin_low : ' + str(asin_low))
