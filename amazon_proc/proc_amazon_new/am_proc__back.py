...
def doTestProc(browser, db_con, db_ali, db_price, asin_low, in_Site, manage_dic):
    time.sleep(2)
    ...
    rtnChk = am_func.proc_asin_parse_brower(asin_low, db_con, db_ali, db_price, browser, "goods", in_Site, manage_dic)
...
if __name__ == '__main__':
    ...
    while flg_m == "0":
        ...
        if in_pgKbn == "test":
            ...
            flg_multi = doTestProc(browser, db_am, db_ali, db_price, asin_list, in_Site, manage_dic)
        elif in_pgKbn == "goods":
            flg_multi = am_func.set_multi(browser, ver, db_am, db_ali, db_price, manage_dic)
        elif str(in_pgKbn).find('stock_out') > -1:
            flg_multi = am_func.set_stock_out(db_am, db_ali, db_price, browser, ver, manage_dic)
        elif in_pgKbn == "stock":
            flg_list = am_func.set_updatelist(db_FS, db_am, db_ali, db_price, browser, ver, manage_dic)
            flg_multi = am_func.set_stock_multi(db_am, db_ali, db_price, browser, ver, manage_dic)
        elif str(in_pgKbn).find('stock_check') > -1:
            flg_list = am_func.set_updatelist(db_FS, db_am, db_ali, db_price, browser, ver, manage_dic)
            flg_multi = am_func.set_stock_multi(db_am, db_ali, db_price, browser, ver, manage_dic)
        # elif str(in_pgKbn).find('old') > -1:
        #     flg_multi = am_func.set_old_multi(db_am, db_ali, db_price, browser, ver, manage_dic)
        else:
            print(">> pgKbn 값을 확인하세요 : {} ".format(in_pgKbn))
            break
        if flg_multi == "1":
