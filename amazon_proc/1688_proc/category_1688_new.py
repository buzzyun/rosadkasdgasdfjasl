...
def newCateDep1(browser):
    ...
    path_file = os.getcwd()
    main_url = "https://114.1688.com/sitemap.html"
    ...
    time.sleep(3)
    with open(path_file + "/1688_proc/temp/result_1688.html","w",encoding="utf8") as f: 
        f.write(str(result))

    if result.find('class="ul-hangye clearfloat"') > -1:
        main_category = getparse(result,'class="ul-hangye clearfloat"','<div class="line-left">')
        sp_bcate = str(main_category).split('<a href=')
        print(">> sp_bcate : {}".format(len(sp_bcate)-1))
        low1 = 0
        for ea_bcate in sp_bcate:
            if str(ea_bcate).find('target=') > -1:
                bcate_url = getparse(str(ea_bcate),'"','"')
                bcate_name = getparse(str(ea_bcate),'>','</a>')
                ...
                tran_bcate_name = tran_bcate_name + str(" NEW")             
                print("\n\n------------------------------------------------------------------------------------------------------------")
...
def newCateDep2(in_drive, in_dep):
    ...
    sql = "select catecode, name, kor_name, amz_cateurl,parent,bcate,mcate,scate,dcate,big,middle,small,little,last,ecate,final,fcate,final1 from t_category where depth = '1' and ishidden='F' and lastcate is null and cate_kbn is null "
    print('\n sql:' + str(sql))
    ...
    while low < len(rows):
        ...
        time.sleep(3)
        with open(path_file + "/1688_proc/temp/result_" +str(bcate_catecode)+ "_stie.html","w",encoding="utf8") as f: 
            ...
            if mcate_source.find('class="ch-menu-item"') > -1:
                ...
                for ea_mcate in sp_mcate:
                    ...
                    if mcate_name != "":
                        ...
                        tran_mcate_name = tran_mcate_name + str(' NEW ')
                        low_2 = low_2 + 1
...
def newCateDep2_new(in_drive, in_dep):
    print('\n [--- newCateDep2 start ---] ')
    last_ck = 0
    path_file = os.getcwd()

    #sql = "select catecode, name, kor_name, amz_cateurl,parent,bcate,mcate,scate,dcate,big,middle,small,little,last,ecate,final,fcate,final1 from t_category where depth = '" + str(int(in_dep)-1) + "' and ishidden='F' and lastcate is null "
    sql = "select catecode, name, kor_name, amz_cateurl,parent,bcate,mcate,scate,dcate,big,middle,small,little,last,ecate,final,fcate,final1 from t_category where depth = '1' and ishidden='F' and lastcate is null and cate_kbn = '1' "
    print('\n sql:' + str(sql))
    ...
    while low < len(rows):
        ...
        # category 방식1
        if result_site.find('class="fd-hide"') > -1:
            print('class="float-cat"')
            mcate_source = getparse(str(result_site),'class="fd-hide"','class="float-cat"')
            mcate_source = getparse(str(mcate_source),'<div class="content">','')
            if mcate_source.find('class="zone-cat') > -1:
                sp_mcate = mcate_source.split('class="zone-cat')
                print(">> sp_mcate ")
                for ea_mcate in sp_mcate:
                    mcate_name = getparse(str(ea_mcate),'<h3 class="ms-yh">','</h3>')
                    mcate_name = getparse(str(mcate_name),'title="','"')
                    if mcate_name != "":
                        tran_mcate_name = ""
                        tran_mcate_name = tran_func.translator(mcate_name, '', 'ko', "")
                        if str(tran_mcate_name).strip() == "":
                        ...
                        tran_mcate_name = tran_mcate_name + str(' NEW ')
                        low_2 = low_2 + 1
                        mcate_url = str(bcate_catecode) + "bcate_" + str(low_2) + "mcate_menu_new"
                        print(">> ===================================================================================================== ")
                        print(">> ( 2 Depth ) ({}) [{}] | {} | {} ".format(low_2, mcate_name, tran_mcate_name, mcate_url))
                        print(">> ===================================================================================================== ")
                        ...
                        if not rs3:
                            ...
                            low3 = 0
                            mcate_item_source = getparse(str(ea_mcate),'class="zone-content fd-clr">','</div>')
                            #mcate_item_source = getparse(str(mcate_item_source),'class="item">','')
                            sp_mcate_item = mcate_item_source.split('</li>')
                            scate_url = ""
                            for ea_item in sp_mcate_item:
                                scate_name  = getparse(str(ea_item),'title="','"')
                                scate_url  = getparse(str(ea_item),'href="','"').replace('amp;','').replace('?&','?')

                                if str(scate_url).find('s.1688.com') == -1:
                                    scate_cnt = scate_cnt + 1
                                    print(">> ( 3 Depth ) ({}) [{}] | {} ".format(low3, scate_name, scate_url))
                                    print(">> s.1688.com 링크 없음 (SKIP) ")
                                    continue
                                else:
                                    scate_url = scate_url.replace('https:','').replace('//s.1688.com','https://s.1688.com')
        ...
        # category 방식2
        elif result_site.find('<div class="mod-category-top fd-clr">') > -1:
            print('<div class="mod-category-top fd-clr">')
            mcate_source = getparse(str(result_site),'<div class="mod-category-top fd-clr">','</div></div>')
            mcate_source = getparse(str(mcate_source),'class="mod-category-item','')
            if mcate_source.find('class="mod-category-item') > -1:
                sp_mcate = mcate_source.split('class="mod-category-item')
                print(">> sp_mcate ")
                for ea_mcate in sp_mcate:
                    mcate_name = getparse(str(ea_mcate),'<dt>','</dt>')
                    mcate_name = getparse(str(mcate_name),'target="_blank">','</a>')
                    if mcate_name != "":
                        tran_mcate_name = ""
                        tran_mcate_name = tran_func.translator(mcate_name, '', 'ko', "")
                        if str(tran_mcate_name).strip() == "":
                        ...
                        if not rs3:
                            ...
                            low3 = 0
                            mcate_item_source = getparse(str(ea_mcate),'<dd>','</dd>')
                            #mcate_item_source = getparse(str(mcate_item_source),'class="item">','')
                            sp_mcate_item = mcate_item_source.split('</a>')
                            scate_url = ""
                            for ea_item in sp_mcate_item:
                                scate_name  = getparse(str(ea_item),'target="_blank">','')
                                scate_url  = getparse(str(ea_item),'href="','"').replace('amp;','').replace('?&','?')
        ...
        # category 방식3
        elif result_site.find('<div class="ch-menu-body">') > -1:
            print('<div class="ch-menu-body">')
            mcate_source = getparse(str(result_site),'<div class="ch-menu-body">','</div></div>')
            mcate_source = getparse(str(mcate_source),'class="ch-menu-item"','')
            if mcate_source.find('class="ch-menu-item-title"') > -1:
                sp_mcate = mcate_source.split('class="ch-menu-item-title"')
                print(">> sp_mcate ")
                for ea_mcate in sp_mcate:
                    mcate_name = getparse(str(ea_mcate),'','</h3>')
                    mcate_name = getparse(str(mcate_name),'<span>','</span>')
                    if mcate_name != "":
                        tran_mcate_name = ""
                        tran_mcate_name = tran_func.translator(mcate_name, '', 'ko', "")
                        if str(tran_mcate_name).strip() == "":
                        ...
                        if not rs3:
                            ...
                            low3 = 0
                            mcate_item_source = getparse(str(ea_mcate),'class="fd-clr">','</ul>')
                            sp_mcate_item = mcate_item_source.split('</li>')
                            scate_url = ""
                            for ea_item in sp_mcate_item:
                                scate_name  = getparse(str(ea_item),'target="_blank">','</a>')
                                scate_url  = getparse(str(ea_item),'href="','"').replace('amp;','').replace('?&','?')
