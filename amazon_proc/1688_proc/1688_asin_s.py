...
class Chrome_drive():
    ...
    def index_page(self,page,url):
        ...
        url = url + str('&beginPage=') + str(page)
...
def main():
    ...
    sql = " select top 1 catecode, kor_name, amz_cateurl, cate_kbn, * from t_category_test where lastcate = '1' and IsHidden = 'F' and cate_kbn = '1' and list_in = '0' order by up_date asc "
    rows = db_con.select(sql)
    if rows:
        for ea_row in rows:
            catecode = ea_row[0]
            kor_name = ea_row[1]
            amz_cateurl = ea_row[2]
            cate_kbn = ea_row[3]
            print(">> ( {} ) {} | {} | {}".format(catecode, kor_name, amz_cateurl, cate_kbn))

            for i in range(1, 6): # 1688 총 6페이지가 표시되었으며 처음 5페이지의 내용이 크롤링
                run.index_page(i,amz_cateurl)
