...
if __name__ == '__main__':
    ...
    for row in range(1, a_class_df.max_row):
        ...
        sqlg = " select cate_hscode from goport_hscode2 where cate_hscode = '{}'".format(cate_hscode)
        row = db_con.selectone(sqlg)
        if not row:
            sqli = "insert into goport_hscode2 (cate_big, cate_name_eng, cate_name, cate_hscode, hidden_hscode, kbn) values ('{}','{}','{}','{}','{}','0')".format(cate_name_big, cate_name_eng, cate_name, cate_hscode, cate_hidden_hscode)
            db_con.execute(sqli)
        else:
            sqlu = "update goport_hscode2 set memo1 = '{}', kbn='2' where cate_hscode = '{}'".format(cate_name, cate_hscode)
            db_con.execute(sqlu)
