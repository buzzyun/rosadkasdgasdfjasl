...
def doTestProc(browser, db_con, db_ali, db_price, in_pgKbn, in_ver, goods_dic, asin_low, manage_dic): #goods_dic, db_con, db_ali, asin_low, connect_mode):
    ...
    # goods test 
    rtnChk = taobao_func_new.proc_asin_parse_brower(goods_dic, db_con, db_ali, browser, pgName, "taobao", db_price, manage_dic)
...
if __name__ == '__main__':
    ...
    input_vpn = str(sys.argv[3]).lower().strip()
    input_type = str(sys.argv[4]).upper().strip() # headless type 여부 : H - headless 
    print(">> SITE : {} | PG NAME : {} | VPN : {} | Type : {} ".format(input_Site, input_pgKbn, input_vpn, input_type))

    if input_Site == 'taobao' or input_Site == 'cn':
        pass
    else:
        print(">> 사이트 값을 확인하세요. {} | {} | {} | {}".format(input_Site, input_pgKbn, input_vpn, input_type))
    ...
    if input_pgKbn == "test":
        sql = " select pgFilename, pgName, now_url, now_url2, target_sql1, isnull(target_sql2,''), isnull(target_sql3,'') from python_version_manage where name = 'goods'"
    else:
    ...
    if str(currIp).strip() == "222.104.189.18":
        ...
        taobao_func_new.version_check_2(db_con, ver, pgFilename, input_pgKbn)
    ...
    while roofCnt < 5000:
        ...
        if input_pgKbn == "test":
            ...
            asin_list = input(" asin@cate_idx@@guid : ")
            print(" input asin_list : " + str(asin_list))
            rtnFlg = doTestProc(browser, db_con, db_ali, db_price, input_pgKbn, ver, goods_dic, asin_list, manage_dic)
        elif input_pgKbn == "goods":
            rtnFlg = taobao_func_new.set_multi(browser, db_con, db_ali, input_pgKbn, ver, goods_dic, db_price, manage_dic)
        elif input_pgKbn == "update":
            print(">> set_updatelist ... ")
            rtnFlg = taobao_func_new.set_updatelist(db_FS, db_con, db_ali, browser, input_pgKbn, ver, goods_dic, db_price, manage_dic)
            time.sleep(180)
        elif str(input_pgKbn).find('stock_out') > -1:
            rtnFlg = taobao_func_new.set_updatelist(db_FS, db_con, db_ali, browser, input_pgKbn, ver, goods_dic, db_price, manage_dic)
            rtnFlg = taobao_func_new.set_stock_out(browser, db_con, db_ali, input_pgKbn, ver, goods_dic, manage_dic)
        elif input_pgKbn == "stock" or input_pgKbn.find("stock_check") > -1:
            rtnFlg = taobao_func_new.set_updatelist(db_FS, db_con, db_ali, browser, input_pgKbn, ver, goods_dic, db_price, manage_dic)
            rtnFlg = taobao_func_new.set_stock_multi(browser, db_con, db_ali, input_pgKbn, ver, goods_dic, db_price, manage_dic)
