...
def get_update_asinset(in_catecode, db_con, db_ali):
    ...
    sql = sql + " where flg_chk ='0' and sitecate = 'cn' and istmall = 'T' "
    sql = sql + " order by RegDate asc "
    rs_main = db_FS.select(sql)
    db_FS.close()
    print('>> ##select all## sql :' + str(sql))
    if rs_main:

        asinCnt = 0
        for ea_asin in rs_main:
            guid = ea_asin[0]
            sql = "select top 100 ali_no, t.isTmall, t.Uid, isnull(t.title,''), t.cate_idx, isnull(t.price, 0), c.bcate \
            ...
            if rs_row:
                ...
                price = rs_row[5]
                bcate = rs_row[6]
                db_stop_update = rs_row[7]
                db_weight = rs_row[8]
                db_Del_Naver = rs_row[9]
                db_goodscode = rs_row[10]
                db_DE_title = rs_row[11]
                db_title = rs_row[12]
                db_minus_opt = rs_row[13]
                db_coupon = rs_row[14]
                db_cate_weight = rs_row[15]
                db_del_flg = rs_row[16]
                db_OriginalPrice = rs_row[17]
                db_price_tmp = rs_row[18]
                db_regdate = rs_row[19]
                db_UpdateDate = rs_row[20]
...
def get_asinset(in_catecode, db_con, db_ali):
    ...
    sql = "select top 100 asin, a.isTmall, t.Uid, isnull(a.title,''), a.catecode, isnull(a.price, 0), c.bcate \
        ...
        , isnull(t.price_tmp,0), t.regdate, t.UpdateDate \
        from T_Category_BestAsin as a inner join T_CATEGORY as c on c.CateCode = a.catecode left join t_goods as t on t.ali_no = a.asin \
        where a.catecode = '{}' and t.uid is null order by newid() ".format(in_catecode)
...
def getGoodsApi(instanceKey, language, header, asinInfo, gDic, db_price, browser):
    ...
    if res.status_code != 200:
        ...
        if taobao_func.findChinese(Title):
            print('>> findChinese Title : {}'.format(Title))
            title_chinese_chk = "1"
            #### return "D02"
            ...
            for ea_value in ConfiguredItems:
                ...
                if float(Quantity) == 0:
                    ...
                    if option_name != "":
                        ...
                        option_value_tran_dic['code'] = str(val_id).strip()
                        option_value_tran_dic['name'] = option_name.strip().replace(" ","&nbsp;")
        ...
        # title Chinese 포함의 경우  처리
        if title_chinese_chk == "1":
            ...
            if option_check == "0":
                ...
                option_item_str = 'N'+ taobao_func.getQueryValue(option_value_tran_arr)

            sql_d = "delete from T_Category_BestAsin_tran where asin = '{}'".format(itemId)
            #print(">> insert sql_i : {}".format(sql_d))
            print(">> del asin : {}".format(itemId))
            db_con.execute(sql_d)

            sql_i = "insert into T_Category_BestAsin_tran (asin, up_date, price, isTmall, title, descript, option_item) values ('{}',getdate(),'{}','{}',{},'{}',{})".format(itemId,mainPrice,isTmall,'N'+taobao_func.getQueryValue(taobao_func.replaceQueryStringTitle(Title)), descript, option_item_str)
            #print(">> insert sql_i : {}".format(sql_i))
            print(">> insert asin : {}".format(itemId))
            db_con.execute(sql_i)

            time.sleep(0.5)
            # tran 타이틀 / 옵션 
            result_tran = taobao_func.procTranConect(browser, itemId, option_val_count)
            time.sleep(0.5)
            tran_title = taobao_func.getTranTitle(result_tran,itemId)
            #print(">> tran_title : {}".format(tran_title))
            if option_check == "1":
                opt_tran_cnt = 0
                tran_option = taobao_func.getTranOption(result_tran,itemId)
                if str(tran_option) != "":
                    sp_tran_option = str(tran_option).split('<input type="hidden"')
                    for ea_tran_item in sp_tran_option:
                        ea_tran_code = ""
                        ea_tran_name = ""
                        ea_tran_code =  taobao_func.getparse(ea_tran_item,'value="','">')
                        ea_tran_name =  taobao_func.getparse(ea_tran_item,'">','').replace("/", "|").replace("&nbsp;", " ").strip()
                        if str(ea_tran_code).strip() != "" and str(ea_tran_name).strip() != "":
                            #print(">> {} : {}".format(ea_tran_code, option_value_dic[ea_tran_code]))
                            if not taobao_func.findChinese(ea_tran_name):
            ...
            if option_check == "1" and opt_tran_cnt == 0:
                # No Option
                print(">> opt_tran_cnt  0 : {}".format(itemId))
            ...
            tran_title = taobao_func.replaceTitle(tran_title, db_ali)
            ...
            if len(tran_title) < 5:
                print('>> title len < 5 ')
                return "D02"
            if taobao_func.findChinese(tran_title):
                print('>> findChinese Title : {}'.format(Title))
                return "D02"

            # title 금지어 체크 ###########
            forbidden_flag = taobao_func.checkForbidden_new(tran_title, db_ali)
            if str(forbidden_flag) != "0":
                print('>> checkForbidden_new : '+str(forbidden_flag))
                return "D03 :" + " ( " + forbidden_flag[2:] + " ) "
...
def proc_Update(instanceKey, language, headers, goods_dic, db_con, db_ali, input_pgKbn, db_price,browser):
    db_FS = DBmodule_FR.Database('freeship')
    global procCnt
    catecode = ""
    get_asin_list = []
    ...
    get_asin_list = get_update_asinset(catecode, db_con, db_ali)
    ...
    for ea_item in get_asin_list:
        ...
        if rtnCheckCode != "":
            ...
            rtnChk = getGoodsApi(instanceKey, language, headers, ea_item, goods_dic, db_price, browser)
        ...
        if rtnChk != "0":  
            ...
            sql = "update amazon_goods_update set flg_chk = '1', upddate = getdate() where guid='{0}'".format(guid)
            print(">> sql : " + str(sql))
        ...
        if rtnChk_no[:1] == "C" or rtnChk_no[:1] == "Q" or rtnChk_no[:1] == "E":
            sql = "update amazon_goods_update set flg_chk = '0', regdate = regdate + 1 where guid='{0}'".format(guid)
            print(">> sql : " + str(sql))
            print(">> amazon_goods_update  : " + str(guid))
            db_FS.execute(sql)
...
def proc_Main(instanceKey, language, headers, goods_dic, db_con, db_ali, input_pgKbn, db_price, browser):
    global procCnt
    catecode = ""
    get_asin_list = []
    pgName = goods_dic['py_pgFilename']
    if input_pgKbn == "test_api":
        ...
        if catecode == "":
            ...
            if len(get_asin_list) == 0:
                ...
                db_con.execute(sql_d)
                sql_d2 = "delete from T_Category_BestAsin where catecode = '{0}'".format(catecode)
...
def getTodayCount(db_con):
    dayCount = 0
    today = datetime.date.today() # YYYY-mm-dd 형태의 시간 출력
    print(">> today : {}".format(today))

    dSql = "delete from python_api_daycount where regdate < '{} 00:00:00'".format(today)
    print(">> dSql : {}".format(dSql))
    db_con.execute(dSql)

    sql = "select isnull(sum(count),0) from python_api_daycount where regdate >= '{} 00:00:00' and regdate <= '{} 23:59:59'".format(today, today)
    rs = db_con.selectone(sql)
    if rs:
        dayCount = rs[0]
...
def insertTodayCount(db_con, procCnt, api_day_count):
    chk = "0"
    print(">> ProcCnt Insert : {}".format(procCnt))
    sql = "insert into python_api_daycount (regdate, count) values (getdate(), {})".format(procCnt)
    print(">> sql : {}".format(sql))
    db_con.execute(sql)

    todayCnt = getTodayCount(db_con)
    if int(todayCnt) > int(api_day_count):
        print(">> (End) todayCnt : {} | api_day_count : {}".format(todayCnt, api_day_count))
...
if __name__ == '__main__':
    ...
    api_day_count = 0
    goods_dic = dict()
    sql = " select pgFilename, pgName, now_url, now_url2, target_sql1, isnull(target_sql2,''), isnull(target_sql3,''), exchange_Rate, dollar_exchange, withbuy_cost, coupon, api_balance, api_balance_date, api_day_count from python_version_manage where name = 'goods_api'"
    rs = db_con.selectone(sql)
    if rs:
        ...
        goods_dic['py_api_day_count'] = rs[13]
        api_day_count = rs[13]
        print(">> api_day_count : {}".format(api_day_count))
        ...
        browser = connectDriverNew("https://www.taobao.com", "")
        ...
        while roofCnt < 100:
            ...
            mainRtn = ""
            try:
                proc_Update(instanceKey, language, headers, goods_dic, db_con, db_ali, input_pgKbn, db_price, browser)
                mainRtn = proc_Main(instanceKey, language, headers, goods_dic, db_con, db_ali, input_pgKbn, db_price, browser)
            except Exception as e:
                procKill()
                db_con.close()
                db_ali.close()
            ...
            # Insert ProcCnt 
            chkFlg = insertTodayCount(db_con, int(procCnt), api_day_count)
            if chkFlg == "1":
                print(">> Today Count Over Exit ")
                flgProc = "1"
                break                
            if mainRtn == "N":
