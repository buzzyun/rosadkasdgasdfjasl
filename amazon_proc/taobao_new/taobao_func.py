...
def procTranConect(browser, in_asin, option_max_count):
    result_tran = ""
    try:
        tran_url = 'https://dev.freeship.co.kr/_GoodsUpdate/title_tran_taobao.asp?asin={0}'.format(in_asin)
    ...
    if dic['OptionKind'] == '300' or dic['OptionKind'] == 300:
        ...
        for key,values in dic['option_img_dic'].items():
            if str(values) == '<br>' or str(values) == '':
                ...
                option_img_set.append('<Font color=blue><pre><b>[ {0} ]</b></pre></FONT><br><img src="{1}"><br><br>'.format(rpl_key,values))
...
def proc_asin_parse_brower(gDic, db_con, db_ali, browser, in_pg, in_pgsite, db_price):
    guid = ""
    asin = gDic['asin']
    ...
    #asin = "615469017341"
    #goods_url = 'https://item.taobao.com/item.htm?id={0}&ns=1'.format(asin)
    goods_url = 'https://www.taobao.com/list/item/{}.htm'.format(asin)
    # print(">> goods_url : {}".format(goods_url))

    # if istmall == 'T':
    #     goods_url = 'https://detail.tmall.com/item.htm?id={}'.format(asin)
    # else:
    #     goods_url = 'https://www.taobao.com/list/item/{}.htm'.format(asin)
    print(">> goods_url : {}".format(goods_url))
    ...
    # with open(path_file + "\goods_" +str(asin)+ ".html","w",encoding="utf8") as f: 
    #     f.write(str(result))
    ...
    if str(browser.current_url).find('tmall') > -1:
        print(">> tmall goods (SKIP): {}".format(asin))
        if result_goods.find('"sold-out-info"') > -1:
            print(">> Tmall 품절(1) : {}".format(goods_url))
            return "T02"
        if result_goods.find('"errorAdvice"') > -1:
            print(">> Tmall 품절(2) : {}".format(goods_url))
            return "T02"
        if result_goods.find('此商品暂时缺货') > -1:
            print(">> Tmall 일시품절: {}".format(goods_url))
            return "T02"
        if result_goods.find('此商品已下架') > -1:
            print(">> Tmall 중단된 상품: {}".format(goods_url))
            return "T02"
        return "T01"

    if str(browser.current_url).find('alicdn.com') > -1:
        print(">> Url No Item alicdn.com (SKIP): {}".format(goods_url))
        return "P01"

    if str(browser.current_url).find('taobao.com/list/item/') > -1 or str(browser.current_url).find('detail.tmall.com/item') > -1:
        if str(browser.current_url).find('taobao.com/list/item/') > -1:
            print(">> taobao item : {}".format(asin))
        elif str(browser.current_url).find('detail.tmall.com/item') > -1:
            print(">> tmall item : {}".format(asin))
            istmall = 'T'
            return "T01"
    else:
        print(">> No item -- taobao (skip - P01)")
        return "P01"
    ...
    if result_goods == "":
        ...
        if str(browser.page_source).find('登录查看更多优惠') > -1 or str(browser.page_source).find('class="rax-view-v2 login"') > -1:
            print(">> Need Login --- ")
            input(">> Please Login :")
            time.sleep(3)
            return "C01"
        ...
        if soldoutCheck(result_goods, istmall) == True:
            ...
            optTitleStr = getparse(result_goods,'<title>','</title>')
            if str(optTitleStr).find('数量') > -1:
                optTitleStr = getparse(optTitleStr,'','数量')
            if str(optTitleStr).find('定金') > -1: # 보증금 체크
                print('>> 定金 (보증금) {}'.format(asin))
                return "D47"
            if str(optTitleStr).find('尾款') > -1: # 결제
                print('>> 尾款 (결제) {}'.format(asin))
                return "D47"

            ##### title #####
            if result_goods.find('<title>') > -1:
                title = getparse(result_goods,'<title>','</title>')
            else:
                title = getparse(result_goods,'item-title">','</div>')
                if title.find('<a ') > -1:
                    title = getparse(result_goods,'">','</a>')
            print(">> title : {}".format(title))

            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> title : " + title
            ...
            ##### price #####
            price = getparse(result_goods,'"itemPrice"','}')
            price = getparse(price,'"price":"','"').strip()
            if price == "" or price.find('-') > -1:
                price = getparse(result_goods,'originalPrice":"','"').strip()
            price = price.replace(',','').strip()
            print(">> price : {}".format(price))
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> price : " + price

            if price == "" and istmall == 'T':
                price = getparse(result_goods,'class="MiniPrice--priceText','</span>')
                price = getparse(price,'">','').strip()
                price = price.replace(',','').strip()
            ...
            other_img_set = []
            if istmall == 'T':
                if result_goods.find('<li class="PicGallery--thumbnail') > -1 :
                    img_str = getparse(result_goods,'<li class="PicGallery--thumbnail','</div>')
                    other_img_list = img_str.split('</li>')
                ...
                if result_goods.find('<img class="item-thumbnail"') > -1 :
                    img_str = getparse(result_goods,'<img class="item-thumbnail"',None)
                    other_img_list = img_str.split('class="item-thumbnail"')
                    for ea_other_img in other_img_list:
                        ea_other_img = getparse(ea_other_img, 'src="', '"')
                        if ea_other_img.find(".jpg") > -1:
                            ea_other_img = getparse(ea_other_img, '', '.jpg') + str('.jpg')
            ...
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> imgB : " + imgB
            ...
            if istmall == 'T':
                ShopId = getparse(result_goods,'class="ShopFloat--enter','<div class="Comments--root')
                ShopId = getparse(ShopId,'href="','.taobao.com').replace("//","")
            else:
                ShopId = getparse(result_goods,'class="shop"','</a>').replace("'","")
                ShopId = getparse(ShopId,'href="','.htm')
                ShopId = getparse(ShopId,'.com/','')
            print(">> ShopId : {}".format(ShopId))
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> imgB : " + imgB

            ### feature ###
            if istmall == 'T':
                feature = getparse(result_goods,'<div class="ItemDetail--','</ul>')
                feature = str(feature).replace('保修期限','').replace('保修期','').replace('保修','').replace('保修期','').replace('全国联保','').replace('售后服务','')
                feature = feature.replace("'","")
                print(">> feature : {}".format(str(feature)[:50]))
            else:
                feature = getparse(result_goods,'<ul class="info ">','</ul>')
                feature = str(feature).replace('保修期限','').replace('保修期','').replace('保修','').replace('保修期','').replace('全国联保','').replace('售后服务','')
                feature = feature.replace("'","")
                print(">> feature : {}".format(str(feature)[:50]))
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> feature : " + feature

            ### descript ###
            descript = ""
            if istmall == 'T':
                descript = getparse(result_goods,'<ul class="info ">','<div id="rate-block"')
                # if str(descript).find('<div class="rax-view-v2 ctrl-bar">') > -1:
                #     descript = getparse(descript,'','<div class="rax-view-v2 ctrl-bar">')
                descript = descript.replace("\&quot;",'').replace("\\",'')
                print(">> descript : {} ".format(str(descript)[:100]))
                # with open(path_file + "\detail_" +str(asin)+ ".html","w",encoding="utf8") as f: 
            #     f.write(str(descript))
            else:
                descript = getparse(result_goods,'<ul class="info ">','<div id="rate-block"')
                descript = descript.replace("\&quot;",'').replace("\\",'')
                print(">> descript : {} ".format(str(descript)[:100]))
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> descript : " + descript

            descript2 = ""
            if result_goods.find('"itemDesc":"') > -1:
                descript2 = getparse(result_goods,'"itemDesc":"',',"itemExist')
            descript2 = descript2.replace("\&quot;",'').replace("&quot;",'').replace("\\",'').replace("'","`")
            print(">> descript2 : {} ".format(str(descript2)[:100]))
            # with open(path_file + "\detail2_" +str(asin)+ ".html","w",encoding="utf8") as f: 
            #     f.write(str(descript2))
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> descript2 : " + descript2
            ...
            result_dic['description'] = descript2
            ...
            option_source = getparse(result_goods,'"skuList":','"skuPVPairList"')
            option_source = str(option_source)
            option_kind_tmp = getparse(result_goods,'skuPropertyList":',']}]}') ##
            sp_option_kind = str(option_kind_tmp).split('"propertyValues"')
            option_kind_name = ""
            for ea_kind in sp_option_kind:
                if ea_kind.find('"pid"') > -1:
                    ea_kind_name = getparseR(ea_kind,'','"pid"') ##
                    ea_kind_name = getparseR(ea_kind_name,'{"name":"','"')
            ...
            if len(sp_option_kind)-1 > 2:
                print(">> sp_option_kind Count : {}".format(len(sp_option_kind)-1))

            option_check = "" # 0 : No Option, 1 : Option Ok, X = Option Skip 
            if option_source.find('"skuId":') == -1:
                option_check = "0"
                print('No Option goods : {} '.format(asin))
                result_dic['OptionKind'] = None
                result_dic['many_option'] = '0'
            else:
                option_sku = option_source
                option_Map = getparse(result_goods,'skuPropertyList":',']}]}') 
                option_check = "1"
                sp_option_item = option_sku.split('},')
                print(">> Option Cnt : {}".format(len(sp_option_item)))

                sp_option_cnt = len(sp_option_item)
            ...
            page_info_tmp = page_info_tmp + "<br><br>--------------------------------------------<br> Option : " 
            ...
            if option_check == "1":
                ...
                opt_tran_cnt = 0
                option_ea_cnt = 0
                for option_ea in sp_option_item:
                    option_value_tran_dic = dict()
                    option_ea_cnt = option_ea_cnt + 1
                    if option_ea_cnt > option_max_count:
                        break
                    #print(">> option_ea : {}".format(option_ea))
                    option_code = getparse(option_ea,'propPath":"','"')
                    option_code_list = option_code.split(';')
                    option_code_count = len(option_code_list)
                    option_price = getparse(option_ea, 'price":"', '"')
                    if option_price == "":
                        option_price = 0
                    else:
                        option_price = float(option_price)
                    option_qty = getparse(option_ea,'quantity":',',').replace('"',"")
                    #옵션 가격 처리
                    ...
                    sku_id = getparse(option_ea, 'skuId":', ',')
                    #print(">> sku_id : {}".format(sku_id))
                    if sku_id == "":
                        pass
                    #elif option_qty == "" or option_qty == "0":
                    #    print(">> Option ({}) (Sold) : [{}] {} | {} ".format(option_ea_cnt, sku_id, option_code, option_price))
                    else:
                        option_value = ""
                        opt_set_cnt = 0
                        for ea_option_code_org in option_code_list :
                            ea_option_code = getparse(ea_option_code_org,':','')
                            find_code = '"vid":'+str(ea_option_code)+'}'
                            #print(">> find_code : {}".format(find_code))
                            if option_Map.find(find_code) > -1 :
                                ea_option_code_value_str = getparseR(option_Map,'',find_code)
                                ea_option_code_value = getparseR(ea_option_code_value_str,'"name":"','"')
                                ea_option_code_value = ea_option_code_value.replace(r'\x26', '&').replace('\xa0', ' ').replace('&#39;', '`').replace('&amp;', '&').replace('&quot;', '').replace("'", "`").replace('"', '')
                                ea_option_code_value = replaceQueryStringOption(ea_option_code_value)

                                if ea_option_code_value.strip() != "":
                                    if option_value.strip() == "":
                                        option_value = ea_option_code_value
                                    else:
                                        option_value = option_value + ' | ' + ea_option_code_value
                                    opt_set_cnt = opt_set_cnt + 1

                            else:
                                #print(">> ea_option_code (Skip) : {}".format(ea_option_code))
                                option_value = ""
                                #break
                                #input(">> Key : ")

                        if len(option_code_list) == opt_set_cnt:
                            print(">> Option ({}) :  {} | {} | {} ".format(option_ea_cnt, ea_option_code, option_value, option_price))
                        else:
                            print(">> Option value Sold : {} | {} ".format(ea_option_code, option_value))

                        if str(option_value).find("预售") > -1:
                            print(">> Option (사전판매/예약) Skip ({}) (option sold) : [{}] {} | {} | {} ".format(option_ea_cnt, sku_id, ea_option_code, option_value, option_price))
                            pass
                        elif str(option_value).find("保修") > -1:
                            print(">> Option (보증) Skip ({}) (option sold) : [{}] {} | {} | {} ".format(option_ea_cnt, sku_id, ea_option_code, option_value, option_price))
                            pass
                        elif str(option_value).strip() == "":
                            #print(">> Option ({}) (option sold) : {} ".format(option_ea_cnt, ea_option_code))
                            print(">> Option ({}) (Sold-value) : [{}] {} | {} | {} ".format(option_ea_cnt, sku_id, ea_option_code, option_value, option_price))
                        if option_qty == "" or option_qty == "0" or str(option_qty) == '"0"':
                            #print(">> Option ({}) (option sold) : {} ".format(option_ea_cnt, ea_option_code))
                            print(">> Option ({}) (Sold-qty) : [{}] {} | {} | {} ".format(option_ea_cnt, sku_id, ea_option_code, option_value, option_price))
                        else:
                            option_val_count = option_val_count + 1
                            option_value = replaceQueryStringOption(option_value)
                            option_code_dic[sku_id] = option_code
                            option_price_dic[sku_id] = float(option_price)
                            option_value_dic[sku_id] = option_value
                            option_value_org_dic[sku_id] = option_value
                            ...
                            #print(">> Option (ok): {} | {}".format(sku_id, option_value))
                            print(">> Option ({}) : [{}] {} | {} | {}".format(option_ea_cnt, sku_id, ea_option_code, option_value, option_price))

                        page_info_tmp = page_info_tmp + "<br>("+str(option_ea_cnt)+") ["+str(sku_id)+"] " + option_value + " | " + str(option_price) + " | ( " + option_code + " )"

                # 옵션 이미지
                if option_kind_tmp.find('{"image"') > -1:
                    option_img_tmp = getparse(option_kind_tmp,'"propertyValues"','')
                    sp_image = option_img_tmp.split('{"image"')
                    for ea_img in sp_image:
                        ea_option_img_url = 'http' + getparse(ea_img,'http','"').strip()
                        ea_option_img_name = getparse(ea_img,'"name":"','"').strip()
                        ea_option_img_name = replaceQueryStringOption(ea_option_img_name)
                        if ea_option_img_url != 'http':
                            option_img_dic[ea_option_img_name] = ea_option_img_url
            ...
            # with open(path_file + "\info_" +str(asin)+ ".html","w",encoding="utf8") as f: 
            #     f.write(str(page_info_tmp))
            ...
            if option_check == "":
                ...
                if str(price) == "":
                    ...
                    sql_i = "insert into T_Category_BestAsin_tran (asin, up_date, price, isTmall, title, descript, option_item) values ('{}',getdate(),'{}','{}',{},'{}',{})".format(asin,price,istmall,'N'+getQueryValue(replaceQueryStringTitle(title)), descript, option_item_str)
                    ...
                    if option_check == "1":
                        ...
                        if str(tran_option) != "":
                            ...
                            for ea_tran_item in sp_tran_option:
                                ...
                                ea_tran_name =  getparse(ea_tran_item,'">','').replace("/", "|").replace("&nbsp;", " ").strip()
...
def set_multi(browser, db_con, db_ali, in_pgKbn, in_ver, goods_dic, db_price):
    ...
    for asin_low in get_asin_list:
        ...
        rtnChk = proc_asin_parse_brower(goods_dic, db_con, db_ali, browser, pgName, "taobao", db_price)  
...
def get_update_goods(in_site, db_FS, db_con):
    ...
    sql = sql + " where flg_chk ='0' and sitecate = 'cn' and isnull(istmall,'') <> 'T' "
...
# Stock ###################################################################################
def set_updatelist(db_FS, db_con, db_ali, in_drive, in_pgsite, in_ver, goods_dic, db_price):
###########################################################################################
    ...
    for asin_low in get_asin_list2:
        ...
        if allCnt == 1 or allCnt == 50:
            procWork(db_con, db_ali, in_drive, "", currIp)
        print('>> version : '+str(in_ver))
        ...
        try:
            rtnChk = proc_asin_parse_brower(goods_dic, db_con, db_ali, in_drive, pgName, "taobao", db_price)  
            print('>> [ rtnChk ] : ' + str(rtnChk))
        except Exception as ex:
        ...
        if rtnChk_no[:1] == "C":
            if c_Errcnt > 15:
                ...
                # proc end
                procEnd(db_con, db_ali, in_drive, pgName)
...
# Stock ##################################################################################################
#def set_stock_multi(db_con, db_ali, in_drive, in_pg, in_pgFilename, in_pgKbn, in_ver, goods_dic, db_price):
...
def proc_asin_out_brower(gDic, db_con, db_ali, browser):
    ...
    result_goods = ""
    time.sleep(3)
    goods_url = 'https://item.taobao.com/item.htm?id=' +str(asin)
    ## goods_url = 'https://www.taobao.com/list/item/{}.htm'.format(asin)
    print(">> goods_url : {}".format(goods_url))
    ...
    if str(browser.current_url).find('taobao.com/list/item/') == -1:
        print(">> Url No Item taobao.com/list/item/ (SKIP): {}".format(goods_url))
        return "P01"
    ...
    if result_goods == "":
        ...
        if result_goods.find('登录查看更多优惠') > -1 or result_goods.find('class="rax-view-v2 login"') > -1:
            print(">> 登录查看更多优惠 --- ")
