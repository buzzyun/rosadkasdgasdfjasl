...
def newlist(db_con, endpage, ip):
    ...
    if not rows:
        ...
        page = 1
        sql = " SELECT top 1 a.amz_cateurl,a.catecode FROM T_CATEGORY as a left outer join update_list b on(a.CateCode=b.catecode) where a.catecode > 10000 and IsHidden='F' and lastcate='1' and category_used_chk is null and b.catecode is null order by up_date asc "
...
def fun_chart(db_con, browser, CateCode, endpage, currIp, file_name, ver, errCnt, sort_type):
    ...
    if str(result).find('<div class="listing-wrap"') == -1:
        ...
        if InternalId.find('otc-') > -1:
            rep_url = "https://open-demo.otcommerce.com/?p=subcategory&cid=" + str(ExternalId)
            try:
                browser.get(rep_url)
            except Exception as e:
                print(">> Check Please 2 Exception Url : {}".format(amzurl))
                print(">> data 없음 (end) : " + str(cateidx))
                procLastpage(cateidx, "end", db_con)
                return "1"
            ...
            if str(result).find('<div class="listing-wrap"') == -1:
                ...
                print(">> Data 있음 : {} | {}".format(cateidx, rep_url))
                sql_s = "select cate_code2 from T_CATEGORY where catecode > 10000 and cate_code2 = '{}'".format(ExternalId)
                rowS = db_con.selectone(sql_s)
                if not rowS:
                    amzurl = rep_url
                    new_catename = getparse(str(browser.page_source),'<h1 class="search-content-info-title">','</h1>')
                    new_catename = new_catename[:90]
                    sql_u = "update T_CATEGORY set amz_cateurl = '{}', used_flg = 'Y', rep_name = '{}' where CateCode = '{}'".format(rep_url, new_catename, cateidx)
                    print(">> catecode [{}] {} -> {} used_flg update ".format(cateidx, cate_korname, new_catename))
                    print("\n--------------------------------------------")
                    db_con.execute(sql_u)
                else:
                    print(">> new category 존재하는 ExternalId : {} | URL : {}".format(ExternalId, rep_url))
                    print(">> data 없음 (end) : " + str(cateidx))
                    procLastpage(cateidx, "end", db_con)
    ...
    #     in_hidden = input(">> Category Hidden Process: ")
    #     if in_hidden == "Y" or in_hidden == "T":
    #         # DB hidden process 
    #         sql_u = "update T_CATEGORY set IsHidden = 'T', used_flg = 'N' where CateCode = '{}'".format(cateidx)
    #         print(">> catecode [{}] hidded update: {}".format(cateidx, sql_u))
    #         db_con.execute(sql_u)
    ...
    #         return "1"
    #     else:
    #         print(">> Pasing Start : {}".format(cateidx))
