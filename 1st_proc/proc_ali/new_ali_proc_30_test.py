...
def proc_order(browser, dic_order):
    ...
    # result = browser.page_source
    # with open(os.getcwd() + "/result_test_ali.html","w",encoding="utf8") as f: 
    #     f.write(str(result))
...
#------------------------------------------------------------
        ...
        print(">> (len) ali 화면 source 전체옵션수 : {}".format(ali_opt_cnt))
    ...
    ###############################################################
    if ali_opt_cnt > 0:
        elem_sku_wrap = browser.find_element(By.CSS_SELECTOR, 'div.sku--wrap--' +str(findClassnameO)) # 옵션전체
        sp_opt_title = str(browser.page_source).split('class="sku-item--property')
        sp_opt_title_cnt = len(sp_opt_title)
        ali_opt_cnt = len(sp_opt_title) -1
    ...
    ##############################################################################################
        ...
        if option_code != "":
            ...
            if cnt_opt_match == ali_opt_cnt:
                ...
                print(">> 옵션코드 옵션명 미매칭 SKIP 대상: {}".format(edit_option))
                proc_LogSet(orderno, goodscode, "X05", msg)
                return "X05"
    ...
    elif str(result_buy).find('<span>주문하기</span>') > -1:
        print(">> 주문하기 버튼 존재 OK ")
    else:
        print(">> No 즉시 구매 버튼 확인불가")
    ...
    if str(browser.page_source).find('밀어서 확인하기') > -1: 
        input(">> (밀어서 확인) 로그인후 아무숫자키 입력하세요 :")
        time.sleep(random.uniform(1, 2))

    elif str(browser.page_source).find('aria-label="비밀번호 찾기"') > -1: 
        try:     
            browser.find_element(By.CSS_SELECTOR,'#fm-history-login-password').send_keys('natto*01')
            time.sleep(1)
            ...
            input(">> 로그인(1)후 아무숫자키 입력하세요 :")
            time.sleep(random.uniform(1, 2))

    if str(browser.page_source).find('밀어서 확인하기') > -1: 
        input(">> (밀어서 확인(2)) 로그인후 아무숫자키 입력하세요 :")
        time.sleep(random.uniform(1, 2))

    elif str(browser.page_source).find('aria-label="비밀번호 찾기"') > -1:
        input(">> 로그인(2)후 아무숫자키 입력하세요 :")
        time.sleep(2)
        result = browser.page_source
        result_org = str(result)
    ...
    # if str(browser.page_source).find('변화') > -1:
    #     print(">> (결제 방법) 변화 ")
    #     payTmp = func_ali.getparse(str(curr_source),'결제 방법','변화')
    #     payTmp = func_ali.getparseR(payTmp,'data-pl="payment-method-title">','</span>')
    #     if payTmp.find(card_lastnum):
    #         print(">> (결제 방법) 선택되어 있음 :{}".format(card_lastnum))
    #     else:
    #         print(">> (결제 방법) 카드 선택 확인필요 ")

    # elif curr_source.find('결제 방법 선택') > -1 or curr_source.find('더보기') > -1:
    ...
    #         # print("\n-----------------------------------")
    #         # print(">> findClassname(2): {}".format(findClassname))
    #         # print("\n-----------------------------------")
...
#################################
    ...
    if ali_opt_cnt > 0:
        ...
        skuAttr = str(skuAttr).replace("%23","#")
        skuAttr = str(skuAttr).replace("%20"," ")
        skuAttr_code = skuAttr.split("#")[0]
        if skuAttr_code.find(option_code) > -1:
            print(">> Option Code Match Ok")
        else:
            print(">> check_skuAttr 확인필요")
            option_ck_flg = "1"
...
if __name__ == '__main__':
    ...
    sql = "select login_id, login_pw, debug_mode, login_mode, ali_id, order_mode, connect_mode, notice_phone \
        , delivery_cost, auto_price_from, auto_price_to, auto_ea_max, auto_max_list \
