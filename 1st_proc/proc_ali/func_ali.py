...
def login_proc(driver, input_id, input_pw):
        result = driver.page_source
        print("팝업 확인")
        chk_pop = PopUpChk(driver, "pop-close-btn")
        if chk_pop == "":
            chk_pop = PopUpChk(driver, "btn-close")
...
def lang_cur_chk(driver):
    if str(driver.current_url).find('gatewayAdapt=glo2kor') > -1 or str(driver.current_url).find('ko.aliexpress.com') > -1:
        pass
        # print(">> 한국 배송 ko.aliexpress ")

    # if str(driver.page_source).find('class="ship-to--menuItem') == -1:
    #     input(">> 한국어 설정 및 USD 통화 설정 하고 숫자키를 입력해 주세요:")
    #     time.sleep(1)
    # else:
    ko_usd_chk = getparse(str(driver.page_source),'span class="ship-to--small','</div>')
    if ko_usd_chk.find('/KO/</span><b>USD') > -1:
        print(">> 한국 설정 / USD 설정 OK ")
    elif ko_usd_chk.find('/KO/') > -1 and ko_usd_chk.find('USD') > -1:
        print(">> 한국 설정 / USD 설정 OK ")
    elif ko_usd_chk.find("KO") > -1 and ko_usd_chk.find('USD') > -1:
        print(">> 한국 설정 / USD 설정 OK ")
    else:
...
# 배송 클릭
def selShipOption(browser, type, delivery_cost):
    flg = "0"
    time.sleep(1)
    ...
    if str(browser.page_source).find('class="product-info"') > -1:
        ...
        elif str(browser.page_source).find('class="dynamic-shipping-line') > -1:
            print(">> selShipOption 4(dynamic-shipping-line)")
            if browser.find_element(By.CSS_SELECTOR,'div.dynamic-shipping-line.dynamic-shipping-titleLayout'):
                browser.find_element(By.CSS_SELECTOR,'div.dynamic-shipping-line.dynamic-shipping-titleLayout').click()
    ...
    cnt_dev = 0
    dev_track_find = "0"
    delivery_source = browser.find_element(By.CSS_SELECTOR,'div.comet-v2-modal-wrap > div > div.comet-v2-modal-content').get_attribute('innerHTML')
    delivery_source = getparse(str(delivery_source),'<div class="dynamic-shipping">','')
    sp_delivery = delivery_source.split('<div class="dynamic-shipping">')
    dic_dev_list = dict()
    for ea_dev in sp_delivery:
        dev_track = getparse(str(ea_dev),'class="dynamic-shipping-mark"','</span>').replace('style="color: rgb(34, 34, 34);">','')
        dev_tmp = getparse(str(ea_dev),'dynamic-shipping-titleLayout">','</div>')
        dev_day = getparse(str(ea_dev),'예상 배송일','</div>').replace('&nbsp;','')
        dev_day = getparse(str(dev_day),'<strong>','</strong>')
        dev_price = ""
        if ea_dev.find('이상 구매 시 무료 배송') > -1:
            print(">> 00 이상 구매 시 무료 배송 ")
            if dev_tmp.find('배송비: US $') > -1:
                dev_price = getparse(str(dev_tmp),'배송비: US $','</span>')
            else:
                dev_price = "1"
        elif ea_dev.find('이상 주문 시 무료배송') > -1:
            print(">> 00 이상 주문 시 무료 배송 ")
            if dev_tmp.find('배송비: US $') > -1:
                dev_price = getparse(str(dev_tmp),'배송비: US $','</span>')
            else:
                dev_price = "1"
        elif dev_tmp.find('무료 배송') > -1:
            dev_price = "0"
        elif dev_tmp.find('배송비: US $') > -1:
            dev_price = getparse(str(dev_tmp),'배송비: US $','</span>')

        print(">> 배송 : [{}] {} | 배송비:{} | {}".format(cnt_dev, dev_track, dev_price, dev_day))
        if dev_track.find('추적 불가') > -1:
            print(">>({}) 추적 불가 (skip) ".format(cnt_dev))
        else:
            dev_track_find = "1"
            if dev_price != "":
                # if float(dev_price) > float(delivery_cost):
                #     print(">> 배송비 초과 (Skip) : {}".format(dev_price))
                # else:
                #     dic_dev_list[cnt_dev] = dev_price
                dic_dev_list[cnt_dev] = dev_price

        cnt_dev = cnt_dev + 1
    print(">> dic_dev_list : {}".format(dic_dev_list))
    ############# input(">> input : ")
    if len(dic_dev_list) == 0:
        if dev_track_find == "1":
            print(">> 배송비 초과 ".format(dev_price))
            return "S021"
        else:
    ...
    print(">> 최저 배송비 위치 : {} | 최저 배송비 : {}".format(sel_dev, min_price))
    shipping_lists = browser.find_elements(By.CSS_SELECTOR,'div.dynamic-shipping-line.dynamic-shipping-additionLayout > span')
    if shipping_lists:
        print(">> 추적 가능 배송 Click ")
        shipping_lists[sel_dev].click()
        flg = "1"
...
def get_codeMsg(code):
    ...
    if code == "S001":
        ...
        rtnMsg = "[{}] 송장처리중 확인필요 ".format(code)
    elif code == "E21":
        rtnMsg = "[{}] 결제카드 확인필요 ".format(code)
    elif code == "E88":
        rtnMsg = "[{}] 네트워크 체크 필요 (결제불가) ".format(code)
    elif code == "E99":
        rtnMsg = "[{}] 기타 확인필요 ".format(code)
    elif code == "0":
        rtnMsg = "[정상] 주문완료 "
