...
def procLogSetDelivery(db_con, proc_log, proc_memo):
    sql = " insert into auto_proc_log_ali (proc_ip, proc_log, proc_memo) "
    sql = sql + " values('" + str(cur_Ip) + "','" + str(proc_log) + "','','','" + str(proc_memo) + "')"
    print(" procLogSetDelivery : " + str(sql))
    db_con.execute(sql)
...
def get_track_name(delicode):
    ...
    # 송장 12자리일 경우
        ...
        if delicode[:2] == "31":
            ...
            DeliveryUid = "6"
            DeliveryName = "FEDEX"                        
        elif delicode[:2] == "56" or delicode[:2] == "57" or delicode[:2] == "58" or delicode[:2] == "59" or delicode[:2] == "43" or delicode[:2] == "50":
    ...
    if not proc_rows:
        ...
        for ea_ord in proc_rows:
            ...
            try:
                ...
                procLogSetDelivery(db_fs, gProc_no, "(proc_delivery) ea_url Exception:"+str(ali_orderno))
                # input(">> ")
            else:
                ...
                ####################################################################################
                # with open(os.getcwd() + "/result_"+str(ali_orderno)+".html","w",encoding="utf8") as f: 
                #     f.write(str(ea_source))
                ####################################################################################
                ...
                ea_track_name = ""
                ea_track_no = ""
                ea_track_name = getparse(ea_source,'carrierTitle','</span>')
                ea_track_name = getparse(ea_track_name,'>','').replace('&nbsp;','').strip()
                ea_track_name = ea_track_name[:30]
                # if ea_source.find('운송장 번호') > -1:
                #     ea_track_no = getparse(ea_source,'운송장 번호','</div>').strip()
                ea_track_no = getparse(ea_source,'mailNoValue','</span>')
                ea_track_no = getparse(ea_track_no,'>','').strip()
                ea_track = ea_track_no.strip()
                print(">>({}/{}) [{}] {} ({})".format(cnt, all_cnt, ali_orderno, ea_track, ea_track_name))

                # track no 
                if regStrChk(ea_track,"KR") == "1": # 한글 있을경우 체크필요:
                    print(">> 한글있음.. Check Please track no : ")
                    procLogSetDelivery(db_fs, gProc_no, "(proc_delivery) 한글있음 Check:"+str(ali_orderno))
                    ####################################################################################
                    input(">> Input : ")
                    ####################################################################################
                # track no 
                if ea_track.find('배송 약속 안내') > -1:
                    print(">> 한글있음 (배송 약속 안내).. Check Please track no : ")
                    procLogSetDelivery(db_fs, gProc_no, "(proc_delivery) 한글있음 (배송 약속 안내):"+str(ali_orderno))
                    ####################################################################################
                    input(">> Input : ")
                    ####################################################################################
                ...
                if ea_track != "":
                    ...
                    if DeliveryUid == "":
                        ...
                        procLogSetDelivery(db_fs, gProc_no, "(proc_delivery) No DeliveryUid Check:"+str(ali_orderno))
...
def proc_cancel(driver, db_fs, ali_id, proc_name, set_diff_day):
    ...
    while date_diff_days < set_diff_day:
        ...
        try:
            ...
            procLogSetDelivery(db_fs, gProc_no, "(proc_cancel) db_save except")
    ...
    if not proc_rows:
        ...
        for ea_ord in proc_rows:
            ...
            try:
                ...
                print(">> proc_finishCheck Exception : {}".format(ali_orderno))
                procLogSetDelivery(db_fs, gProc_no, "(proc_finishCheck) url Check:"+str(ali_orderno))
                input(">> 배송 추적 Check please : ")
            else:
                ...
                if regStrChk(ea_track,"KR") == "1": # 한글 있을경우 체크필요:
                    print(">> 한글있음.. Check Please track no : ")
                    procLogSetDelivery(db_fs, gProc_no, "(proc_finishCheck) 한글있음 Check")
                    ####################################################################################
                    input(">> Input : ")
                    ####################################################################################
                # track no 
                if ea_track.find('배송 약속 안내') > -1:
                    print(">> 한글있음 (배송 약속 안내).. Check Please track no : ")
                    procLogSetDelivery(db_fs, gProc_no, "(proc_finishCheck) 한글있음 (배송 약속 안내)")
                    ####################################################################################
                    input(">> Input : ")
                    ####################################################################################
