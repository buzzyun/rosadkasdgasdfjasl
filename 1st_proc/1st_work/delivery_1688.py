            ...
            if ea_order != "":
                ...
                if ea_order.find('/ship/trk/tracking-details?') == -1:
                    ...
                    if orderTrackNo == "":
                        ...
                        if orderGoodNo == "":
                            sql = "select i.uid, o.uid, o.orderno, o.state, isnull(d.tracking_china,''), d.tracking_china_date, tracking_china_state from t_order as o inner join t_order_info as i on i.OrderUid = o.uid join T_ORDER_DELIVERY as d on d.uid = i.uid \
                            where state in ('201','301','421') and ali_id = 'ref' and tracking_china_state is null and ali_orderno = '{}'".format(orderEbayNo)
                        else:
                            sql = "select i.uid, o.uid, o.orderno, o.state, isnull(d.tracking_china,''), d.tracking_china_date, tracking_china_state from t_order as o inner join t_order_info as i on i.OrderUid = o.uid join T_ORDER_DELIVERY as d on d.uid = i.uid \
                            where state in ('201','301','421') and ali_id = 'ref' and tracking_china_state is null and ali_orderno = '{}' and order_ali_no = '{}' ".format(orderEbayNo, orderGoodNo)
                        rows = db_con.select(sql)
...
def procReturnCancelPage(browser):
    ...
    for page in range(1,procPage+1):
        ...
        browser.get(linkUrl)
        time.sleep(5)
        if str(browser.page_source).find(' Canceled</h2>') == -1:
            time.sleep(5)
        func_user.getparse(str(browser.page_source), 'title-container', 'class="m-container__footer"')
        orderResult = func_user.getparse(str(browser.page_source), ' Canceled</h2>', 'class="m-container__footer"')
        orderSP = orderResult.split('<div class="m-ph-card m-order-card">')
        print(">> orderSP : {}".format(len(orderSP)))
        ordCnt = 0
        for ea_item in orderSP:
            if ea_item != "":
                ...
                if str(stateTmp) == "Order canceled" or str(stateTmp) == "Order returned":
                    print(" >> {} proc ".format(stateTmp))
                    # ebay cancelled
                    if str(stateTmp) == "Order canceled": reason = "ebay_cancel"
                    if str(stateTmp) == "Order returned": reason = "ebay_returncancel"
                    if str(cancelOrderNo) != "":
                        sql_q = " SELECT O.orderno, O.Uid , I.Uid, O.state from T_ORDER_info I inner join T_ORDER_DELIVERY D on I.Uid=D.Uid \
                        ...
                        if not row_q:
                            ...
                            if not row:
                                ...
                                iSql = iSql + " values ( getdate(), '" + str(db_orderno) + "','" + str(cancelOrderNo) + "','ref', '" + str(reason) + "', '" + str(db_Ouid) + "', '" + str(db_InfoUid) + "' )"
...
if __name__ == '__main__':
    ...
    upd_cnt = 0
    print('>> 1688 Delivery Get Proc (1688 송장 번호 --> 프리쉽 또는 배대지 송장 입력 처리) ')
    print('>> [--- main start ---] ' + str(datetime.datetime.now()))

    cur_Ip = socket.gethostbyname(socket.gethostname())
    # func_user.procLogSet(db_con, gProc_no, "S", "0", "1688 현지 송장 가져오기 : "+str(cur_Ip))
...
################################################################################
# https://trade.1688.com/order/buyer_order_list.htm?spm=&scene_type=&source=&purchase_company_name=null&product_name=&start_date=&end_date=&seller_login_id=&trade_status=&trade_type_search=&biz_type_search=&order_id_search=&is_his=&is_hidden_canceled_offer=&apt=&related_code=&order_settle_flag=&company_name=&keywords=&receiver_tel=&receiver_name=&buyer_name=&down_stream_order_id=&batch_number=&total_fee=&page=3&page_size=10
################################################################################

    now_url = "https://trade.1688.com/order/buyer_order_list.htm"
    browser.get(now_url)
    time.sleep(random.uniform(4,6))
    if str(browser.page_source).find('class="title order-title"') > -1:
        print(">> 로그인 된 상태 ")
        time.sleep(2)
    else:
        print(">> After Login Input (2) : ")
        time.sleep(1)
        now_url = "https://trade.1688.com/order/buyer_order_list.htm"
        browser.get(now_url)
        time.sleep(random.uniform(4,6))
    ...
    with open(os.getcwd() + "/result_1688_orderlist1.html","w",encoding="utf8") as f: 
    ...
    if str(browser.page_source).find('class="title order-title"') == -1:
        print(">> Login Failed ")
        input(">> ")
        #func_user.procLogSet(db_con, gProc_no, "E", "0", "login 불가")
    ...
    result = str(browser.page_source)
    with open(os.getcwd() + "/result_1688_orderlist2.html","w",encoding="utf8") as f: 
        f.write(str(result))

    input(">> Input : ")
    ## https://trade.1688.com/order/new_step_order_detail.htm?orderId=2360816977054260354&tracelog=20120313bscentertologisticsbuyer&#logisticsTabTitle
    order_list = ['2360816977054260354','2359655582327260354','2360527718806260354']
    for ea_order in order_list:
        print(">> Order No : {}".format(ea_order))
        order_url = "https://trade.1688.com/order/new_step_order_detail.htm?orderId="+str(ea_order)+"&tracelog=20120313bscentertologisticsbuyer&#logisticsTabTitle"

        browser.get(order_url)
        time.sleep(random.uniform(3,5))
        if str(browser.page_source).find('物流编号：') > -1:
            print(">> 송장 번호 있음 ")
    
        result_order = str(browser.page_source)
        with open(os.getcwd() + "/result_1688_order"+str(ea_order)+".html","w",encoding="utf8") as f: 
            f.write(str(result_order))

    
    
    # func_user.procLogSet(db_con, gProc_no, "F", "0", "END 1688 현지 송장 가져오기 : "+str(cur_Ip))
